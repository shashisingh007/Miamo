...
The "tell my parent" doc. Single sitting. Covers the 4 layers (tracking → learning → ranking → composing), the algorithms at a glance (v3.5's 17 baseline + v3.6 v8 modules + v3.7 v9 Temporal Learning + Phase E's 5 new algorithms), what stays private, what's on the roadmap.
...
11 services + 19 worker loops (13 v3.5 + 4 v3.6.0 + 1 v3.7 Temporal Learning + 1 G.18 activation-emails) + the request lifecycle. Mermaid diagrams for every major flow.
...
Phone → ingest envelope (32 KB cap, 50 events max, v=1) → Redis stream `events:raw` → 19 worker loops → Postgres aggregates → algorithms read. All 57+ event types catalogued (Phases D-G and session 13/14 added `preference.drift_detected`, `discover.seeded_fallback`, `intent.override_applied`, plus the Phase F coming-soon events). All 19 workers with schedules. The HMAC privacy story.
...
Local dev. **The bootstrap trap** (`set -a; source .env; set +a` before `start.sh local dev` — otherwise v8 flags don't load). Production topology (k8s, 3 envs). CI/CD. Secrets. Prometheus metrics. The 19 worker loops table. Migration flow. Cold storage.
...
**Real-user scenario:**
> An engineer on a Mac wants to test the v3.6.0 Move v2 composer locally. They `colima start`, `docker compose up postgres redis`, `cd services/shared && npx prisma migrate dev`, copy `.env.example` to `.env`, set `FEATURE_MOVE_V2_ENABLED=1`. Crucially, before `miamo start`, they run `set -a; source .env; set +a` (the bootstrap trap — `start.sh` doesn't source `.env`). They open http://localhost:3100, log in as `miamo10@miamo.test`, match with miamo20, tap ✨Suggest, see 5 suggestions. If they forgot the `source .env`, the endpoint would return 404 and the suggestions wouldn't appear.
...
Two subdirectories under `docs/` contain reference material that lives outside the 14-doc reading sequence:

- **`docs/architecture/`** — deeper design memos: the v3.6 overhaul design and the competitive market scan that informed it, plus the per-phase status docs shipped through Phases D-G and session 13/14 (see `docs/architecture/all-phases-complete-status.md` for the master summary, and `docs/architecture/session-13-status.md` for the latest coming-soon-feature wiring). Linked from PRODUCT.md and OWNER_GUIDE.md where relevant. Read these when you want to understand *why* a v3.6 decision was made.
