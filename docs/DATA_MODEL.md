...
# v3.7+ / Phase D / Phase G / session 13 schema additions

Between v3.6.0 (2026-06) and v1.2-dev (2026-07) the schema picked up
three more tables and three additional columns. The migrations that
ship these changes are:

- `services/shared/prisma/migrations/2026063…_v9_temporal_learning_v2/migration.sql` — Phase D `UserPreferenceHistory`.
- `services/shared/prisma/migrations/2026070…_g16_notification_prefs/migration.sql` — per-channel notification granularity columns on `Settings` + `NotificationPreference` if separate.
- `services/shared/prisma/migrations/20260703000000_v12_admin_dtm_intent/migration.sql` — session 13 additions (idempotent `ADD COLUMN IF NOT EXISTS` + `CREATE TABLE IF NOT EXISTS`, FK guarded via DO blocks).

| Change | What it adds | Introduced |
|---|---|---|
| New: `UserPreferenceHistory` | Phase D — 5-window EMA per (userId, dimension). Written every 6h by `preferenceWindows.ts`. Drift detector reads short-vs-long ratios. | v3.7 Phase D |
| New: `DtmInterest` | Session 13 Task 3a — DTM-side mutual-interest record. Unique `(fromUserId, toUserId)`. Rows survive DTM completion. | Session 13 |
| New: `DtmMatch` | Session 13 Task 3a — DTM mutual-interest match. Canonical ordering `(user1Id, user2Id)` via sort — same pattern as `Match`. Created transactionally when both `DtmInterest` rows exist. | Session 13 |
| Extended: `User` | `isAdmin` (default FALSE) — gates `/api/v1/admin/fairness-gini` and any future admin surface. | Session 13 Task 3c |
| Extended: `Settings` | `manualIntentOverride` (nullable enum, one of the 7 `IntentClass` ids) — user-visible override of the app's inferred right-now intent. Consumed by the social ranker when `FEATURE_INTENT_VISIBILITY_ENABLED=1` (session 14 Task 2). | Session 13 Task 3b |
| Extended: `Settings` | Per-channel notification granularity columns (`push*`, `email*`, `inApp*` toggles) — feeds the per-channel opt-out UI. | Phase G.16 |
