...
Miamo is a **TypeScript monorepo of eleven services** sharing one Postgres 16 database, one Redis 7 cache, one canonical Prisma schema, and one tracking pipeline. The web app (Next.js 14 App Router) talks only to the **gateway** (port 3200) — never directly to a domain service. The gateway verifies the JWT, applies a stack of seven rate limiters, sets a strict CSP via helmet, then proxies the request to one of six domain services (auth, users, social, messaging, content, notifications). A separate **ingest** edge (port 3260) accepts tracking events from the same gateway origin, validates a 64-KB envelope, and writes to a Redis stream `events:raw`. A **tracking-worker** (port 3261) consumes the stream and runs **nineteen background loops** (13 from v3.5 + 4 new in v3.6.0 + 1 v3.7 Phase D `preferenceWindows` + 1 G.18 `activationEmails`) that fold raw events into hourly/daily aggregates, embeddings, pair-compat scores, exposure-credit ledgers, weekly stable-match outputs, and outgoing activation touchpoints. The **shared** library (no port) holds the canonical Prisma schema (71 models — 67 baseline + 4 new in v3.6.0), 53 v3.5 + 17 v3.6 = 70 algorithm modules, and every middleware primitive (validate, sanitize, errorHandler, requestId, idempotency, metrics, internal-auth).
...
| **tracking-worker** | 3261 | 13 v3.5 + 4 v3.6.0 + 1 v3.7 (preferenceWindows) + 1 G.18 (activationEmails) = **19 total** |
...
- **File:** `services/tracking-worker/src/index.ts` (entry) + 19 worker modules under `services/tracking-worker/src/` (v3.7 added `preferenceWindows.ts`, G.18 added `activationEmails.ts`)
...
flowchart TB
  ...
  subgraph Worker["Async tracking pipeline"]
    direction LR
    TW["tracking-worker :3261<br/>19 loops (13 v3.5 + 4 v3.6 + 1 v3.7 preferenceWindows + 1 G.18 activationEmails)"]
  end
  ...
  ING -- "XADD events:raw" --> RD
  TW -- "XREADGROUP events:raw" --> RD
  TW -- "writes aggregates · 19 loops" --> PG
...
| **v3.7 Phase D — Temporal Learning v2** | | | | | | |
| 18 | **PreferenceWindows** | `services/tracking-worker/src/preferenceWindows.ts` | 6h (all users, 4-window drift scan) | `ALGO_V9_TEMPORAL_LEARNING_ENABLED` | `EventAggDaily` (30/60/90d + all-time), `UserPreferenceHistory` | `UserPreferenceHistory` snapshots + emits `preference.drift_detected` per user when the drift magnitude clears the confidence threshold |
| **G.18 — Activation touchpoints** | | | | | | |
| 19 | **ActivationEmails** | `services/tracking-worker/src/activationEmails.ts` | 5 min (schedule table lookup) | `FEATURE_ACTIVATION_EMAILS_ENABLED` | `User.createdAt`, `Match`, `Message`, `Notification` (past-touchpoint state) | Queues templated welcomes at 0h / 24h / 48h / day-7 via the notifications service. Idempotent by `(userId, template)` guard. |
