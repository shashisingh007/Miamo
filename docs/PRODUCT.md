...
A worker (`services/tracking-worker/`) runs 17 loops (13 v6/v6.5/v6.6/v7 + 4 v3.6.0: intentInference, exposureScheduler, stableMatchTop10, fairnessAudit) that roll those events into Postgres aggregates: hourly counts (`EventAggHourly`), daily counts with target sets (`EventAggDaily`), per-user feature snapshots (`FeatureSnapshot`), and pairwise compatibility caches (`PairCompatCache`). See `docs/ARCHITECTURE.md:115` for the canonical count.
