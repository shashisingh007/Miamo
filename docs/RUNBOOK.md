...
cd ../..
miamo stop            # 2. stop all tsx-watch processes
miamo start           # 3. restart
...
# Appendix E — How to verify graceful degradation (chaos scripts)

Phase G.7 ships four chaos scripts under `scripts/chaos/`. Run these
before every launch to confirm the "one dependency dying does not
cascade" contract still holds.

## The four scripts

| Script | What it proves |
|---|---|
| `scripts/chaos/kill-postgres.sh`     | Postgres kill → 5xx during outage, `/healthz` recovers within 30 s of restart. |
| `scripts/chaos/kill-redis.sh`        | Redis kill → services stay 200 (fail-open per audit). Rate-limit + idempotency + geo-cache all tolerate ECONNREFUSED. |
| `scripts/chaos/partition-network.sh` | Simulated partition to postgres → 503 not 500, auto-recovery on heal. |
| `scripts/chaos/oom-tracking-worker.sh`| SIGKILL tracking-worker twice → docker restarts it, no crash loop. |

## Prereq

Stack up on docker-compose:
...
Each script is idempotent. Any script that exits non-zero is a real
regression — see `scripts/chaos/README.md` for the interpretation guide.

    miamo chaos kill-redis          # ~15 s
    miamo chaos kill-postgres       # ~45 s
    miamo chaos partition-network   # ~60 s
    miamo chaos oom-tracking-worker # ~45 s

## What "graceful degradation" means in Miamo

- **Redis** is a soft dep. Losing it must not 5xx a request. Rate-limit
  middleware and idempotency middleware both catch ECONNREFUSED and
  fall through (allow the request). Geo-cache falls through to network.
- **Postgres** is a hard dep for anything that reads/writes user data,
  but `/healthz` on the gateway proper does NOT touch the DB — a
  transient DB outage must not remove the gateway from the load
  balancer.
- **Tracking-worker** is stateless. Events are durable in the Redis
  stream; on restart the worker resumes at the last-committed offset.

## When to run

- **Before every major release** — the launch-critical gate.
- **After changes to** `services/shared/src/service.ts` or any
  middleware layer (rate-limit, idempotency).
- **After a dependency bump** on `ioredis`, `@prisma/client`, or `express`.
