...
- Stack up: `bash scripts/start.sh local dev` (web on :3100, gateway on :3200).
...
### Run
    ...
    LOAD_TOKEN='<bearer>' LOAD_CHAT_ID='<uuid>' bash scripts/load/run.sh messages
...
```bash
bash scripts/rollback.sh <sha12>          # docker compose path
bash scripts/rollback.sh <sha12> --k8s    # kubernetes path
```
