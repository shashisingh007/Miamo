...
# Terminal 2 — start the mobile bundler (auto-detects your LAN IP)
miamo mobile start
```
...
`miamo mobile start` auto-fills `EXPO_PUBLIC_API_URL` with your Mac's LAN IP so the phone can reach the backend running on your Mac. Every save on your Mac hot-reloads on your phone in ~1s.
