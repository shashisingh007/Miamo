// Miamo Mobile — Shared TypeScript interfaces.
// Ported verbatim from services/web/src/lib/types.ts so response shapes
// match. If a web endpoint changes shape, update both files together (grep
// for the interface name across the repo before editing).
...
export interface MiamoProfile {
  ...
  diet: string;
  // ─── Optional server-side fields the mobile onboarding + profile edit
  // read/write. Kept optional because the shape returned by /api/v1/profiles/me
  // is a superset that includes these; the interface widening keeps callers
  // strict without forcing every consumer to guard.
  displayName?: string;
  interests?: string[];
  heightCm?: number;
  job?: string;
  photos?: Array<{ id?: string; url: string; position?: number }>;
  prompts?: Array<{ question: string; answer: string; position?: number }>;
}
