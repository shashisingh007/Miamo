...
// was captured at module load; now resolved per-call so late-hydrated
// secrets (secrets.ts) take effect without a restart.
let _cachedHashSecret: string | null = null;
function resolveHashSecret(): string {
  const cur = process.env.TRACKING_HASH_SECRET || 'dev-only-tracking-hash-secret-change-me';
  if (cur !== _cachedHashSecret) _cachedHashSecret = cur;
  return cur;
}
...
function hashTid(id: string): string {
  if (!id) return '';
  return createHmac('sha256', resolveHashSecret()).update(id).digest('base64url').slice(0, 22);
}
