...
export function devClearCodes() { devCodeCache.clear(); }
...
export function verifyChallengeToken(token: string, expectedDeviceHash: string): { userId: string } {
  ...
  if (sig !== expected) throw new OtpError('CHALLENGE_INVALID', 'Invalid challenge token', 400);
  if (Date.now() > parseInt(expStr, 10)) throw new OtpError('CHALLENGE_EXPIRED', 'Challenge expired, please log in again', 401);
...
function parseSignupToken(token: string, expectedKind: SignupKind): { channel: OtpChannel; identifier: string } {
  ...
  if (sig !== expected) throw new OtpError('SIGNUP_TOKEN_INVALID', 'Invalid signup token', 400);
  if (Date.now() > parseInt(expStr, 10)) throw new OtpError('SIGNUP_TOKEN_EXPIRED', 'Signup session expired, start over', 401);
