...
/**
 * Clamp an untrusted query/body value to an integer within [min, max]. Returns
 * `def` for missing / non-finite / non-numeric input. Bug-hunt fix #14 + #15
 * (docs/architecture/bug-hunt-2026-07.md) — replaces raw `parseInt` on age /
 * distance / weight filters that used to accept `Infinity`, `NaN`, negative
 * values, and out-of-range integers.
 */
export function clampInt(v: unknown, min: number, max: number, def?: number): number | undefined {
  const n = typeof v === 'string' ? parseInt(v, 10) : typeof v === 'number' ? v : NaN;
  if (!Number.isFinite(n)) return def;
  const clamped = Math.min(max, Math.max(min, Math.floor(n as number)));
  return clamped;
}

/**
 * Clamp an untrusted numeric value to a float within [min, max]. Rejects
 * `Infinity`, `-Infinity`, `NaN`. Used by distance filters (km) and any other
 * user-supplied float range.
 */
export function clampFloat(v: unknown, min: number, max: number, def?: number): number | undefined {
