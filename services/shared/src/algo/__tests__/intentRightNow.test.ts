...
describe('inferIntent — properties', () => {
  ...
  it('events older than 5min are ignored for general classes', () => {
    const events: RecentEvent[] = [
      ev('click.rage', 6 * 60 * 1000),
      ev('click.rage', 7 * 60 * 1000),
      ev('click.rage', 8 * 60 * 1000),
      ev('swipe.regret', 9 * 60 * 1000),
    ];
    const v = inferIntent({ lastNEvents: events, viewerFeatures: {}, nowMs: NOW });
