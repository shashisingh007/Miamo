...
describe('FibonacciHeap', () => {
  it('empty', () => {
    const h = new FibonacciHeap<number, string>(cmp);
  ...
  it('extractMin returns smallest', () => {
    const h = new FibonacciHeap<number, string>(cmp);
    [5, 1, 3].forEach((k) => h.insert(k, String(k)));
    expect(h.extractMin()).toEqual({ key: 1, value: '1' });
  });

  it('peek does not remove', () => {
    const h = new FibonacciHeap<number, number>(cmp);
    h.insert(7, 7);
    h.peek();
    expect(h.size()).toBe(1);
  });
  ...
  it('handles 500 ascending', () => {
    const h = new FibonacciHeap<number, number>(cmp);
    for (let i = 0; i < 500; i++) h.insert(i, i);
    let prev = -1;
    while (!h.isEmpty()) {
  ...
  it('string comparator', () => {
    const h = new FibonacciHeap<string, number>((a, b) => a.localeCompare(b));
    ['banana', 'apple', 'cherry'].forEach((k, i) => h.insert(k, i));
  ...
  it('random workload yields sorted extractions', () => {
    const h = new FibonacciHeap<number, number>(cmp);
    let seed = 12345;
    const data: number[] = [];
    for (let i = 0; i < 200; i++) {
