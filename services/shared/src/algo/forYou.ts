...
export type ForYouInputs = {
  ...
  /**
   * v2: audit flagged the universal `2 * log1p(impressionsLast48h)` fatigue
   * as too aggressive for premium users, who deliberately browse more
   * per session. When `isPremium=true`, the fatigue term is scaled by
   * PREMIUM_FATIGUE_FACTOR (0.7) before subtraction. Default undefined
   * = false, so the ranker's output is unchanged for non-premium users
   * and for callers that don't populate this field.
   */
  isPremium?: boolean;
};

/**
 * v2: premium users get a 30% fatigue reduction. Enters as a multiplier
 * on the fatigue penalty so a premium user who has seen 20 impressions
 * eats 0.7× the penalty a free user would. // because [audit §E.2 #7]:
 * premium users report the app "goes dead" after 10 minutes because the
 * ranker over-suppresses on repeat impressions.
 */
export const PREMIUM_FATIGUE_FACTOR = 0.7;

/** Pure helper — computes fatigue with the premium reduction applied. */
export function fatiguePenalty(impressions48h: number, isPremium?: boolean): number {
  const base = 2 * Math.log1p(impressions48h);
  return isPremium ? base * PREMIUM_FATIGUE_FACTOR : base;
}
