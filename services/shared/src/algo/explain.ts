...
import { v9CompatibilityExplainerEnabled } from './flags';
import { explainCompatibility, type ExplainerReason } from './v9/compatibilityExplainer';
...
/**
 * v2: v9 companion — natural-language explainer output. Behind
 * `ALGO_V9_COMPATIBILITY_EXPLAINER_ENABLED`. When the flag is OFF,
 * returns `null` so callers fall back to the classic star/breakdown
 * card. When ON, returns up to `max` short template-composed reasons
 * (see `services/shared/src/algo/v9/compatibilityExplainer.ts`).
 *
 * // because [audit §E.2 #4]: the v2 "why am I seeing this" card asks
 * // for readable sentences, but we don't want to pay the flag-flip
 * // twice — the same `Explain` breakdown feeds both the legacy formatter
 * // and the new one.
 */
export function explainToReasons(e: Explain, max = 3): ExplainerReason[] | null {
  if (!v9CompatibilityExplainerEnabled()) return null;
  const ingredients: Array<{ name: string; contribution: number; detail?: string }> = [];
  for (const [name, raw] of Object.entries(e.breakdown)) {
    if (raw == null) continue;
    const value = Number(raw);
    if (!Number.isFinite(value)) continue;
    const w = (e.weights as Record<string, number>)[name] ?? 0;
    if (w <= 0) continue;
    const contribution = value * w;
    if (contribution <= 0) continue;
    ingredients.push({ name, contribution });
  }
  return explainCompatibility({ ingredients, maxReasons: max });
}
