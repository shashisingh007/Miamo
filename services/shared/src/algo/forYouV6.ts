...
import { learnerRamp, applyLearnerRamp, v9TemporalLearningEnabled } from './flags';
import { attentionFit, hesitationFit, chronoOverlap, intentMatchScore, fatiguePenalty, type ForYouInputs, type Explain } from './forYou';
import { V9_DRIFT_DAMPEN_STRENGTH } from './v8/multiObjective';
...
export type ForYouV6Inputs = ForYouInputs & {
  ...
  /**
   * v2: v9 Phase D drift-magnitude for the candidate's primary dimension,
   * [0,1]. When ALGO_V9_TEMPORAL_LEARNING_ENABLED=1 and this value is
   * non-null, we apply the same drift-dampen the multi-objective ranker
   * uses (V9_DRIFT_DAMPEN_STRENGTH = 0.15 → up to 15% reduction on a
   * strongly-cooling dimension). When the flag is off (default), the
   * value is ignored; forYouV6 output is unchanged.
   */
...
export function scoreForYouV6(inp: ForYouV6Inputs): { score: number; explain: Explain } {
  ...
  // v2: fatigue reduced by 30% for premium users — see forYou.fatiguePenalty.
  const fatigue = fatiguePenalty(impressionsLast48h, inp.isPremium);

  // v2: v9 drift-dampen — when the temporal-learning flag is on AND the
  // caller supplied a non-null drift magnitude, apply the same 0..15%
  // multiplicative dampener the multi-objective ranker uses. This keeps
  // the two v9 consumption paths (v8 ranker + v6 discover) coherent: a
  // cooling dimension nudges both. When the flag is off, the compose is
  // untouched — bit-identical to the pre-v2 behaviour.
  const driftFactor = v9TemporalLearningEnabled() && inp.driftMagnitude != null
    ? 1 - Math.max(0, Math.min(1, inp.driftMagnitude)) * V9_DRIFT_DAMPEN_STRENGTH
    : 1;
  ...
  if (pair && Date.now() - new Date(pair.computedAt).getTime() < CACHE_FRESH_MS) {
    ...
    const score = clip100(base * driftFactor * 100 - fatigue - regretPenalty - repeatPassPenalty - wsDamp + returnBoost);
    return {
      score,
      explain: {
  ...
  const raw = compose(breakdown, W) * driftFactor * 100;
