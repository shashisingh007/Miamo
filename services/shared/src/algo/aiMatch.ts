...
import { v5FeatureEnabled, v9MatchQualityPredictorEnabled } from './flags';
import {
  predictMatchQuality,
...
/**
 * v2: notification-priority hint. At MATCH creation, the notifications
 * service can pass this pair's features to get an `immediate | delayed |
 * lowest` tier and decide push-now vs batch vs digest. Not part of the
 * ranker scoring — it's an ordering hint. Behind
 * `ALGO_V9_MATCH_QUALITY_PREDICTOR_ENABLED`; returns `null` when off so
 * the notification path falls back to the legacy "push immediately"
 * behaviour.
 *
 * // because [audit §E.2 #2]: today every match wakes the user with a
 * // push even if compatibility is 0.2 and the receiver has a 0.8 ghost
 * // rate. This is the cheapest lever to reduce notification-blindness.
 */
export function predictMatchNotificationPriority(input: MatchQualityInput): MatchQualityResult | null {
  if (!v9MatchQualityPredictorEnabled()) return null;
  return predictMatchQuality(input);
}
