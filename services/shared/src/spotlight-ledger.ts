 ...
 *
 * bug-hunt fix #13 (docs/architecture/bug-hunt-2026-07.md #33) — added the
 * `MAX_REFUND_MINUTES` sanity ceiling. Even though callers today all pass a
 * small integer, an accidental `Number.MAX_SAFE_INTEGER` from a buggy meta-
 * data path used to be storable. Now such calls throw + log + refuse.
 */
export const MAX_REFUND_MINUTES = 1000;
...
export async function refund(
  ...
  if (amount > MAX_REFUND_MINUTES) {
    logger.error(`refund: attempted refund of ${amount} minutes exceeds MAX_REFUND_MINUTES=${MAX_REFUND_MINUTES}; reason=${reason} user=${userId}`);
    throw new Error(`refund: amount ${amount} exceeds MAX_REFUND_MINUTES (${MAX_REFUND_MINUTES})`);
  }
