...
// v3.7 Temporal Learning v2 — emitted server-side by the preferenceWindows
// worker when a drift signal clears the magnitude + confidence thresholds.
// Payload matches DriftSignal (algo/v9/driftDetector.ts) with the four
// window scores omitted (they're kept in FeatureSnapshot.raw.drift).
export const PreferenceDriftDetectedSchema = z.object({
  dimension: z.string().min(1).max(64),
  driftMagnitude: z.number().min(0).max(1),
  driftDirection: z.enum(['cooling', 'warming', 'stable']),
  confidence: z.number().min(0).max(1),
}).strict();

// ─── Phase F — coming-soon features (client-emitted) ────────────────
// Every field is strictly typed + bounded. Payloads are minimal because
// the server-side write already carries the full record; these events are
// funnel signals, not the source of truth.

export const AccountDeleteInitiatedSchema = z.object({
  step: z.enum(['open_modal', 'type_confirm', 'submit']),
  reasonCategory: z.enum(['not_using', 'found_partner', 'privacy', 'harassment', 'temp_break', 'other']).optional(),
}).strict();

export const AccountDeleteCompletedSchema = z.object({
  elapsedMs: z.number().int().nonnegative().max(60_000),
}).strict();

export const AccountExportDownloadedSchema = z.object({
  bytes: z.number().int().nonnegative().max(1024 * 1024 * 200), // hard-cap 200MB
  tables: z.number().int().nonnegative().max(50),
}).strict();

export const SafetyReportSubmittedSchema = z.object({
  reasonId: z.enum([
  ...
  targetType: z.enum(['user', 'message', 'photo', 'story', 'post', 'creativity']),
  hasEvidence: z.boolean(),
}).strict();

export const SafetyBlockBulkUnblockSchema = z.object({
  count: z.number().int().min(1).max(200),
}).strict();

export const FamilyBriefSharedSchema = z.object({
  channel: z.enum(['whatsapp', 'copy_link', 'other']),
  format: z.enum(['pdf', 'image', 'text']),
}).strict();

export const FamilyBriefDashboardViewedSchema = z.object({
  totalShares: z.number().int().nonnegative().max(500),
  activeShares: z.number().int().nonnegative().max(500),
}).strict();

export const TrustScoreViewedSchema = z.object({
  score: z.number().int().min(0).max(100),
  tier: z.enum(['unverified', 'starter', 'trusted', 'verified']),
  badgeEligible: z.boolean(),
}).strict();

export const WeeklyTopCountdownExpiredSchema = z.object({
  weekIso: z.string().min(6).max(12),
}).strict();

// G.18 Task 1b — emitted from social's GET /api/v1/discover when the
// seeded fallback tops up an under-filled candidate pool. Growth reads
// this via the daily rollup to size how often day-1 users hit the seed.
export const DiscoverSeededFallbackSchema = z.object({
  naturalCount: z.number().int().nonnegative().max(500),
  seededCount: z.number().int().nonnegative().max(50),
  preferCity: z.string().max(128).nullable().optional(),
}).strict();

// v1.2 session 14 — Task 2 — emitted from social's GET /api/v1/discover
// when FEATURE_INTENT_VISIBILITY_ENABLED=1 AND the requester has a
// manualIntentOverride set. Payload: the applied override + the inferred
// class (if any) it replaced. Growth reads this via the daily rollup to
// size how often users override the inferred class.
export const IntentOverrideAppliedSchema = z.object({
  override: z.enum([
    'distraction_browse',
    'intentional_browse',
    'reply_mood',
  ...
  inferred: z.string().max(64).nullable().optional(),
}).strict();
...
export const V6_VALIDATORS = {
  ...
  // v3.7 v9 — Temporal Learning v2
  'preference.drift_detected':   PreferenceDriftDetectedSchema,
  // Phase F — coming-soon features shipped end-to-end
  'account.delete_initiated':      AccountDeleteInitiatedSchema,
  'account.delete_completed':      AccountDeleteCompletedSchema,
  'account.export_downloaded':     AccountExportDownloadedSchema,
  'safety.report_submitted':       SafetyReportSubmittedSchema,
  'safety.block_bulk_unblock':     SafetyBlockBulkUnblockSchema,
  'family_brief.shared':           FamilyBriefSharedSchema,
  'family_brief.dashboard_viewed': FamilyBriefDashboardViewedSchema,
  'trust_score.viewed':            TrustScoreViewedSchema,
  'weekly_top.countdown_expired':  WeeklyTopCountdownExpiredSchema,
  // G.18 activation-funnel wiring
  'discover.seeded_fallback':      DiscoverSeededFallbackSchema,
  // v1.2 session 14 — Task 2: ranker consumer of manualIntentOverride
  'intent.override_applied':       IntentOverrideAppliedSchema,
