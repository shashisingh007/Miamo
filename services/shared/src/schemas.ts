...
// v3.7 signup/signin rewrite — clients may now send `identifier` (a raw
// email OR phone), `email`, or `phone`. Server auto-detects channel from
// format and normalises. Keeping optional aliases lets old clients that
// still POST `{email,password,...}` keep working. `.strict()` still rejects
// unknown fields so the boundary invariant holds.
const identifierRawSchema = z.string().trim().min(1).max(254);
const phoneRawSchema = z.string().trim().min(3).max(32);

export const registerBodySchema = z
  .object({
    email: emailSchema.optional(),
    phone: phoneRawSchema.optional(),
    identifier: identifierRawSchema.optional(),
    password: passwordSchema,
    displayName: displayNameSchema,
  })
  .strict()
  .refine((v) => !!(v.email || v.phone || v.identifier), {
    message: 'email, phone, or identifier required',
    path: ['identifier'],
  });
...
// v3.7 — passwordless login (OTP challenge for an existing account).
// Stage 1: send OTP; Stage 2: verify OTP and mint session tokens.
export const loginOtpStartBodySchema = z
  .object({
    identifier: identifierRawSchema.optional(),
    email: emailSchema.optional(),
    phone: phoneRawSchema.optional(),
...
export const loginOtpVerifyBodySchema = z
  .object({
    otpToken: z.string().min(1).max(2048),
    code: z.string().trim().regex(/^\d{6}$/, 'code must be 6 digits'),
  })
  .strict();

// v3.7 — OTP-only signup completion (no password chosen). Reuses the
// verified-token from /signup/verify to bind the account creation to a
// verified email/phone. Server generates a random unusable passwordHash;
// the user can set a real password later via forgot-password.
export const signupOtpCompleteBodySchema = z
  .object({
    verifiedToken: z.string().min(1).max(2048),
    displayName: displayNameSchema,
  })
  .strict();
...
// Accept both `toUserId` (canonical, matches /like) and `userId` (legacy
// client naming). Handler normalises to toUserId.
export const discoverPassBodySchema = z.object({
  toUserId: z.string().min(1).max(64).optional(),
  userId: z.string().min(1).max(64).optional(),
  reason: z.string().max(200).optional(),
}).strict().refine(v => v.toUserId || v.userId, {
  message: 'toUserId (or userId) required',
  path: ['toUserId'],
});
...
// Accept both `url` (canonical) and `videoUrl` (what several clients + docs use).
// Handler normalises to `url`.
export const videoBodySchema = z.object({
  ...
  url: z.string().trim().min(1).max(5_000_000).optional(),
  videoUrl: z.string().trim().min(1).max(5_000_000).optional(),
  thumbnailUrl: z.string().trim().max(5_000_000).optional().nullable(),
  category: z.string().max(80).optional(),
  visibility: z.enum(['everyone', 'matches', 'private']).optional(),
}).strict().refine(v => v.url || v.videoUrl, {
  message: 'url (or videoUrl) required',
  path: ['url'],
});
...
// Accept both `toUserId` (canonical) and `targetUserId` (client convention).
export const accessRequestCreateBodySchema = z.object({
  toUserId: z.string().min(1).max(64).optional(),
  targetUserId: z.string().min(1).max(64).optional(),
  field: z.enum(ACCESS_FIELDS),
  message: z.string().trim().max(500).optional(),
}).strict().refine(v => v.toUserId || v.targetUserId, {
  message: 'toUserId (or targetUserId) required',
  path: ['toUserId'],
});
