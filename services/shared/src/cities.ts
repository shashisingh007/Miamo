...
// Miamo's primary market is India. When two cities tie on match quality,
// prefer India (e.g. "Bangalore, IN" over "Bangkok, TH" for query "bang").
// Explicit country weight bonus applied before population.
function countryBias(country: string): number {
  if (country === 'IN' || country === 'India') return 1_000_000_000; // dominant
  return 0;
}
...
export function searchCities(query: string, limit = 12): City[] {
  ...
  const rank = (a: City & { key: string }, b: City & { key: string }) => {
    const bias = countryBias(b.country) - countryBias(a.country);
