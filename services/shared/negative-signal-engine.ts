...
export function buildNegativeProfile(events: NegativeEvent[]): NegativeSignalProfile {
  ...
  const sev = (kind: NegativeEvent['kind']): number => {
    if (kind === 'report') return 3.0;
    if (kind === 'block') return 2.5;
    if (kind === 'unmatch') return 1.5;
    return 1.0;
  };
  ...
  for (const e of events) {
    ...
    for (const [k, v] of Object.entries(e.targetTraits)) {
      ...
      const key = `${k}:${String(v).toLowerCase()}`;
  ...
  const max = Math.max(1, ...Object.values(accum));
