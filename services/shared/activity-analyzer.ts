...
export class UserActivityAnalyzer {
  ...
  buildContentTasteProfile(): ContentTasteProfile {
    ...
    for (const a of this.activities) {
      ...
      // Category engagement
      if (meta.category) {
        const weight = a.action === 'content_engage'
          ? (meta.engageAction === 'share' ? 5 : meta.engageAction === 'comment' ? 4 : meta.engageAction === 'like' ? 2 : 1)
          : 1;
      ...
      // Author engagement
      if (meta.authorId && a.action === 'content_engage') {
        authorScores[meta.authorId] = (authorScores[meta.authorId] || 0) + 1;
      }
