...
export function mountCreativitySpotlight(
  ...
  // MUST gate behind a verified Razorpay / IAP receipt at
  // `POST /api/v1/payments/spotlight/{order,verify}` (which today returns 501
  // — real provider ships in v1.1). The sandbox path is a launch-blocker if
  // it stays open in production because any authenticated user could grant
  // themselves +180 minutes for free.
  // bug-hunt fix #11 (docs/architecture/bug-hunt-2026-07.md #21, P0) — refuse
  // to grant minutes when NODE_ENV === 'production'.
      ...
      if (process.env.NODE_ENV === 'production') {
        return res.status(501).json({
          error: {
            ...
            message: 'Payment integration ships in v1.1. Use /api/v1/payments/spotlight/verify with a Razorpay receipt.',
            statusCode: 501,
          },
        });
      }
  ...
  // ─── Save / unsave ────────────────────────────────
  // bug-hunt fix #2 (docs/architecture/bug-hunt-2026-07.md #2) — the previous
  // implementation did `findUnique` → conditional `delete` OR `create`, both
  // outside a transaction. Two concurrent taps both saw `existing=null` and
  // both called `create`, causing a P2002 unique-constraint 500 and an
  // inconsistent `saveCount`. Now the read + toggle + count-update run inside
  // a single `$transaction()` and P2002 on create is caught + folded into the
  ...
  app.post('/api/v1/creativity/items/:id/save', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const outcome = await prisma.$transaction(async (tx) => {
        const existing = await tx.creativitySave.findUnique({
          where: { itemId_userId: { itemId, userId } },
        });
        if (existing) {
          await tx.creativitySave.delete({ where: { id: existing.id } });
        ...
        try {
          await tx.creativitySave.create({ data: { itemId, userId } });
        } catch (err: any) {
          // P2002: another parallel save won the race. Treat as "already saved"
          // — no count double-increment, no client-visible 500.
          if (err?.code === 'P2002') return { saved: true } as const;
          throw err;
        }
        await tx.creativityItem.update({
          where: { id: itemId },
