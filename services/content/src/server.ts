...
import { feedPostBodySchema, feedPostUpdateBodySchema, reactionBodySchema, commentBodySchema, storyBodySchema, storyReactBodySchema, videoBodySchema, showcaseCreateBodySchema, showcaseUpdateBodySchema, SHOWCASE_LINK_ALLOWLIST, deferCreateBodySchema, deferListQuerySchema, deferResolveBodySchema, DEFER_PILE_CAP, moveSuggestionsV2BodySchema, familyBriefGenerateBodySchema, dtmMutualInterestBodySchema } from '../../shared/src/schemas';
import crypto from 'crypto';
import { extractSenderVoice, type OutboundMessageSample } from '../../shared/src/algo/v8/moveV2/senderVoice';
import { buildResonance, type FirstMoveOutcomeSample, type OpenerKind, type ResonanceTone } from '../../shared/src/algo/v8/moveV2/receiverResonance';
import { compose as moveCompose, type ComposerInput } from '../../shared/src/algo/v8/moveV2/composer';
import { detectLanguageFamily, type LanguageFamily } from '../../shared/src/algo/v8/moveV2/codeMix';
import { rankHooks, type HookCandidate } from '../../shared/src/algo/v8/moveV2/hookLibrary';
import { festivalHooksForToday } from '../../shared/src/algo/v8/festivalHooks';
import { buildDtmFeed, type DtmTopicCandidate, type DtmTopicHistory } from '../../shared/src/algo/dtmFeedV7';
import { buildMaskedDtmFeed, maskRejectedTopics } from '../../shared/src/algo/v8/dtmBatch';
import { dtmColdStart } from '../../shared/src/algo/dtmColdStart';
import { DTM_TOPIC_KEYS, type DtmTopicKey } from '../../shared/src/algo/dtmTopics';
import { MOOD_TTL_MS } from '../../shared/src/algo/v8/moodRightNow';

// ─── v3.6.0 — Move v2 & Family Brief flag helpers ───
function isMoveV2Enabled(): boolean { return process.env.FEATURE_MOVE_V2_ENABLED === '1'; }
function isFamilyBriefEnabled(): boolean { return process.env.FEATURE_FAMILY_BRIEF_ENABLED === '1'; }

// ─── v8 — DTM topic-mask flag helper ────────────────
// Default OFF: when unset or any value other than '1', the mask is not applied
// and `/api/v1/dtm/next-batch` returns the byte-identical v7 ranking. This is
// the hard "ALGO_V8_DTM_MASK_ENABLED" gate referenced in dtmTopicMask.ts §D.11.
function isDtmMaskEnabled(): boolean { return process.env.FEATURE_DTM_MASK_ENABLED === '1'; }

// ─── Phase G.11 — Text moderation flag helper ───────
// Default OFF: when unset or any value other than '1', the text moderator is
// bypassed on POST creativity + POST message. When ON, bad text is either
// soft-blocked (400 → user re-edits) or hard-blocked (audit-logged) based on
// severity. See docs/architecture/moderation-pipeline.md.
function isTextModerationEnabled(): boolean { return process.env.FEATURE_TEXT_MODERATION_ENABLED === '1'; }
import { sanitize, sanitizeObject } from '../../shared/src/sanitize';
import { toPublicUser } from '../../shared/src/security/publicUser';
import { recomputeAndPersistCompletion } from '../../shared/src/completion';
...
import { paymentInitLimiter, webhookLimiter } from '../../shared/src/rateLimits';
...
import {
  mountCreativitySpotlight, startTrendingWorker,
  onBeatCreated, onMoveSent,
} from './creativity-spotlight';
import {
  awardFirstPostInCategory,
  awardCommentLeft, awardReactionReceived, awardFirstAction,
  spend, refund, getBalance, isValidPostMinutes,
  MIN_MINUTES, MINUTES_STEP, MAX_MINUTES_PER_POST,
} from '../../shared/src/spotlight-ledger';
import { recordCreativityAction } from '../../shared/src/creativity-track';
import { getTextModerator } from '../../shared/src/moderation/textModerationClient';
...
const sentry = installSentry({ serviceName: 'content' });
...
app.get('/api/v1/feed', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    let result = posts.map(p => {
      const author = toPublicUser(p.author as any);
...
app.post('/api/v1/feed', authMiddleware, idempotency(), validate({ body: feedPostBodySchema }), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const author = toPublicUser(post.author as any);
...
app.get('/api/v1/stories', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    for (const s of stories) {
      const author = toPublicUser(s.author as any);
...
app.post('/api/v1/stories', authMiddleware, idempotency(), validate({ body: storyBodySchema }), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
...
app.get('/api/v1/videos', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const userId = req.userId!;
    const result = videos.map(v => { const author = toPublicUser(v.author as any); const { reactions, ...rest } = v; return { ...rest, author, liked: reactions.some(r => r.userId === userId), likeCount: v._count.reactions, commentCount: v._count.comments }; });
...
app.post('/api/v1/videos', authMiddleware, validate({ body: videoBodySchema }), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { title: rawTitle, description: rawDescription, url, videoUrl, thumbnailUrl, category, visibility } = req.body;
    const title = sanitize(rawTitle || '');
    const description = sanitize(rawDescription || '');
    // Accept either `url` (canonical) or `videoUrl` from the client.
    const finalUrl = url || videoUrl;
    const video = await prisma.video.create({ data: { authorId: req.userId!, title, description, url: finalUrl, thumbnailUrl, category: category || 'general', visibility: visibility || 'everyone' } });
    res.json({ data: video });
  } catch (e) { next(e); }
});
...
app.get('/api/v1/creativity/feed', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    // "followedAuthors" proxy: Miamo has no Follow model — users with whom the viewer has an
    // active Match are the closest equivalent (mutual relationship → preferred content boost).
    ...
    for (const m of myMatches) followedAuthors.add(m.user1Id === userId ? m.user2Id : m.user1Id);
    ...
    const scored = candidates.map(item => {
      ...
      const author = toPublicUser(item.author as any);
      return {
        id: item.id,
        authorId: item.authorId,
...
app.post('/api/v1/creativity/items/:id/hide', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    // Persistent pass signal — analyzer treats this as a strong negative on category/author.
    const itemMeta = await prisma.creativityItem.findUnique({
      where: { id: req.params.id },
      select: { category: { select: { name: true } } },
    });
    recordCreativityAction(prisma, req.userId!, 'creativity.pass', req.params.id, {
      category: itemMeta?.category?.name,
      authorId: item.authorId,
    });
...
app.post('/api/v1/creativity/items/:id/move', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    // Persist the move action and propagate creator-trait learning + match milestones.
    recordCreativityAction(prisma, req.userId!, 'creativity.move', req.params.id, {
      authorId: item.authorId,
      mutual: !!mutual,
      matched: !!match,
      hasMessage: !!message,
    });
    onMoveSent(prisma, req.userId!, req.params.id, !!match && !!mutual, item.authorId)
      .catch((e) => logger.warn('onMoveSent failed', e));
...
app.post('/api/v1/creativity/items/:id/share', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    recordCreativityAction(prisma, userId, 'creativity.share', itemId, {
      category: itemMeta?.category?.name,
      authorId: itemMeta?.authorId,
      channel: req.body?.channel,
    });
    // Spotlight: first-share one-time award.
    if (itemMeta && itemMeta.authorId !== userId) {
      awardFirstAction(prisma, userId, 'share_sent').catch((e) => logger.warn('awardFirstAction share_sent failed', e));
    }
...
app.get('/api/v1/creativity/items', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const userId = req.userId!;
    const result = items.map(i => { const author = toPublicUser(i.author as any); const { reactions, ...rest } = i; return { ...rest, author, liked: reactions.some(r => r.userId === userId), likeCount: i._count.reactions, commentCount: i._count.comments, viewCount: i._count.viewRecords }; });
    res.json({ data: result, cursor: items[items.length - 1]?.id });
  } catch (e) { next(e); }
});

app.post('/api/v1/creativity/items', authMiddleware, idempotency(), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    let { categoryId, category, type, title, content, description, mediaUrl, visibility, featured, mediaType, minutesPaid, thumbnailUrl, durationSec, collabIds } = req.body;
    title = sanitize(title || '');
    content = sanitize(content || '');
    description = sanitize(description || '');
    if (category) category = sanitize(category);

    // ─── Phase G.11 — Text moderation (flag-gated) ────
    // Default OFF via FEATURE_TEXT_MODERATION_ENABLED. When ON, we run the
    // combined title + caption + description through the shared moderator.
    // - hard-severity decisions → 422 (audit-logged, rejected outright)
    // - soft-severity decisions → 400 (user re-edits inline)
    // We do NOT partially block: any hit rejects the whole submission so
    // the client sees a single, coherent error.
    if (isTextModerationEnabled()) {
      const combined = [title, content, description].filter(Boolean).join('\n').trim();
      if (combined) {
        const decision = await getTextModerator().moderateText(combined);
        if (!decision.approved) {
          if (decision.severity === 'hard') {
            await auditLog(prisma, req.userId!, 'moderation.creativity.hard_block', {
            ...
            return res.status(422).json({
              error: {
                ...
                message: 'This post cannot be published.',
                categories: decision.categories,
              },
            });
          }
          // Soft block — user can re-edit
          return res.status(400).json({
            error: {
              code: 'CONTENT_NEEDS_EDIT',
              message: 'Please edit your post before publishing.',
              categories: decision.categories,
              severity: 'soft',
            },
          });
    ...
    // ─── Spotlight: minute-spend gate (multiple of 5, min 5, max 60) ──
    // Posts created without `minutesPaid` keep legacy behaviour (no debit, no expiry)
    // so existing client builds still work; new clients should always send it.
    const requestedMinutes = Number(minutesPaid ?? 0);
    let appliedMinutes = 0;
    let expiresAt: Date | null = null;
    if (requestedMinutes > 0) {
      if (!isValidPostMinutes(requestedMinutes)) {
        return res.status(400).json({
          error: { message: `minutesPaid must be a multiple of ${MINUTES_STEP} between ${MIN_MINUTES} and ${MAX_MINUTES_PER_POST}` },
        });
      }
      try {
        await spend(prisma, req.userId!, requestedMinutes, 'post_spend', null, { categoryId });
        appliedMinutes = requestedMinutes;
        expiresAt = new Date(Date.now() + requestedMinutes * 60_000);
      } catch (err: any) {
        if (err?.code === 'INSUFFICIENT_BALANCE') {
          return res.status(402).json({ error: { code: 'INSUFFICIENT_BALANCE', message: 'Not enough Spotlight minutes', balance: err.balance } });
        }
        throw err;
      }
    }

    const cleanCollabIds: string[] = Array.isArray(collabIds)
      ? Array.from(new Set(collabIds.filter((id: unknown): id is string => typeof id === 'string' && id !== req.userId!))).slice(0, 5)
      : [];

    const resolvedMediaType = (mediaType === 'image' || mediaType === 'video' || mediaType === 'text') ? mediaType : (type || 'text');

    // bug-hunt fix #12 (docs/architecture/bug-hunt-2026-07.md #32) — if the
    // item.create fails after a successful spend(), issue a compensating
    // refund so the user does not lose Spotlight minutes to a create-time
    // crash. Refund uses a distinct reason so ledger analysis can spot
    // failure hotspots.
    let item: any;
    try {
      item = await prisma.creativityItem.create({
      data: {
        authorId: req.userId!, categoryId,
        type: type || resolvedMediaType,
        mediaType: resolvedMediaType,
        title: title || 'Untitled',
        content: itemContent,
        mediaUrl,
        thumbnailUrl: thumbnailUrl || null,
        durationSec: typeof durationSec === 'number' ? Math.max(0, Math.min(60, Math.floor(durationSec))) : null,
        visibility: visibility || 'everyone',
        featured: featured || false,
        minutesPaid: appliedMinutes,
        expiresAt,
        collabIds: cleanCollabIds,
        status: 'live',
      },
      ...
      if (appliedMinutes > 0) {
        try {
          await refund(prisma, req.userId!, appliedMinutes, 'refund_post_failed', null, { originalReason: 'post_spend', categoryId });
          logger.warn(`creativity.item.create failed after spend; auto-refunded ${appliedMinutes} minutes for user=${req.userId}`);
        } catch (refErr) {
          logger.error(`creativity.item.create failed AND refund failed for user=${req.userId} minutes=${appliedMinutes}: ${(refErr as Error).message}`);
        }
      }
      throw createErr;
    }

    // First-post-in-category bonus (idempotent).
    let firstPostBonus: { granted: boolean; balanceAfter: number } | null = null;
    if (item.category?.name) {
      try { firstPostBonus = await awardFirstPostInCategory(prisma, req.userId!, item.category.name); } catch {}
    }

    // Persistent learning hook — every upload writes UserActivity row.
    recordCreativityAction(prisma, req.userId!, 'creativity.upload', item.id, {
    ...
    res.json({ data: item, spotlight: { spent: appliedMinutes, expiresAt, firstPostBonus } });
  } catch (e) { next(e); }
});

app.post('/api/v1/creativity/items/:id/react', authMiddleware, idempotency(), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const itemId = req.params.id;
    const userId = req.userId!;
    const itemMeta = await prisma.creativityItem.findUnique({
      where: { id: itemId },
      select: { authorId: true, status: true, category: { select: { name: true } } },
    });
    const existing = await prisma.creativityReaction.findUnique({ where: { itemId_userId: { itemId, userId } } });
    let liked: boolean;
    if (existing) {
      await prisma.creativityReaction.delete({ where: { id: existing.id } });
      await prisma.creativityItem.update({ where: { id: itemId }, data: { beatCount: { decrement: 1 } } }).catch(() => {});
      liked = false;
      recordCreativityAction(prisma, userId, 'creativity.unlike', itemId, {
        category: itemMeta?.category?.name,
        authorId: itemMeta?.authorId,
      });
      res.json({ data: { liked: false } });
    } else {
      await prisma.creativityReaction.create({ data: { itemId, userId, type: req.body.type || 'like' } });
      liked = true;
      trackActivity(prisma, userId, 'like', 'creativity', itemId);
      recordCreativityAction(prisma, userId, 'creativity.like', itemId, {
        category: itemMeta?.category?.name,
        authorId: itemMeta?.authorId,
        trending: itemMeta?.status === 'trending',
      });
      // Spotlight earn paths: viewer gets first_beat_sent one-time;
      // author gets +1/reaction up to daily cap. Both fire-and-forget.
      if (itemMeta && itemMeta.authorId !== userId) {
        awardFirstAction(prisma, userId, 'beat_sent').catch((e) => logger.warn('awardFirstAction beat_sent failed', e));
        awardReactionReceived(prisma, itemMeta.authorId, itemId, userId).catch((e) => logger.warn('awardReactionReceived failed', e));
      }
      res.json({ data: { liked: true } });
    }
    await recalcTrend(itemId);
    if (liked && itemMeta && itemMeta.authorId !== userId) {
      onBeatCreated(prisma, userId, itemId).catch((e) => logger.warn('onBeatCreated failed', e));
    }
  } catch (e) { next(e); }
});

app.post('/api/v1/creativity/items/:id/comments', authMiddleware, idempotency(), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const itemId = req.params.id;
    const userId = req.userId!;
    const comment = await prisma.creativityComment.create({
      data: { itemId, authorId: userId, content: sanitize(req.body.content || '') },
      include: { author: { select: { id: true, displayName: true, username: true, verified: true, photos: { take: 1, orderBy: { position: 'asc' } } } } },
    });
    await recalcTrend(itemId);
    const itemMeta = await prisma.creativityItem.findUnique({
    ...
    recordCreativityAction(prisma, userId, 'creativity.comment', itemId, {
      category: itemMeta?.category?.name,
      authorId: itemMeta?.authorId,
      length: comment.content.length,
    });
    // Spotlight: first-comment one-time + daily-capped comment_left award.
    if (itemMeta && itemMeta.authorId !== userId) {
      awardFirstAction(prisma, userId, 'comment_sent').catch((e) => logger.warn('awardFirstAction comment_sent failed', e));
      awardCommentLeft(prisma, userId, itemId).catch((e) => logger.warn('awardCommentLeft failed', e));
    }
    res.json({ data: comment });
  } catch (e) { next(e); }
});
...
app.post('/api/v1/creativity/items/:id/view', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const itemId = req.params.id;
    const userId = req.userId!;
    const durationMs = Number(req.body?.durationMs ?? req.body?.dwellMs ?? 0) || 0;
    await prisma.creativityView.upsert({ where: { itemId_viewerId: { itemId, viewerId: userId } }, create: { itemId, viewerId: userId }, update: {} });
    await prisma.creativityItem.update({ where: { id: itemId }, data: { views: { increment: 1 } } });
    await recalcTrend(itemId);
    trackActivity(prisma, userId, 'view', 'creativity', itemId);
    const itemMeta = await prisma.creativityItem.findUnique({
      where: { id: itemId },
      select: { authorId: true, category: { select: { name: true } } },
    });
    recordCreativityAction(prisma, userId, 'creativity.view', itemId, {
      category: itemMeta?.category?.name,
      authorId: itemMeta?.authorId,
    }, durationMs);
    res.json({ data: { success: true } });
...
// Allowed columns on MatrimonialProfile — enumerated from prisma schema.
// Prisma 500s on any unknown key (`email`, `hobbies`, `profilePhoto`, etc.),
// which caused every "populate DTM" PUT with extra fields to explode.
const MATRIMONIAL_STRING_COLS = new Set([
  'fullName','birthTime','birthPlace','height','weight','complexion','bloodGroup',
  'bodyType','physicalStatus',
  'religion','caste','subCaste','gotra','manglik','star','raasi','dosham','motherTongue',
  'education','educationDetail','college','occupation','company','annualIncome','workingCity','workingCountry',
  'fatherName','fatherOccupation','motherName','motherOccupation',
  'familyType','familyStatus','familyValues','nativePlace','familyIncome',
  'maritalStatus','diet','drinking','smoking',
  'kundliUrl','kundliData','nakshatra',
  'aboutMe','aboutFamily',
  'partnerHeightMin','partnerHeightMax','partnerReligion','partnerCaste',
  'partnerEducation','partnerOccupation','partnerIncome','partnerCity',
  'partnerManglik','partnerMaritalStatus','partnerMotherTongue','partnerDiet',
  'partnerExpectation',
  // v3.2 extended prefs
  'partnerSmoking','partnerDrinking','partnerFamilyType','partnerFamilyValues',
  'partnerLocations','partnerRelocate','partnerChildren',
  'bioDataTemplate',
  'phoneNumber','alternatePhone','linkedIn','contactEmail',
]);
const MATRIMONIAL_INT_COLS = new Set([
  'brothers','brothersMarried','sisters','sistersMarried',
  'numerologyNumber','destinyNumber','soulNumber',
  'partnerAgeMin','partnerAgeMax',
]);
const MATRIMONIAL_BOOL_COLS = new Set([
  'horoscopeMatch','bioDataGenerated',
  'bioDataPublic','phonePublic','linkedInPublic','emailPublic','photosPublic',
]);
...
app.put('/api/v1/matrimonial/profile', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const raw = sanitizeObject(req.body) as Record<string, unknown>;
    const data: Record<string, unknown> = {};
    // Whitelist + type-coerce every accepted column. Everything else is
    // dropped silently (was previously causing 500s from Prisma when the
    // client sent `email`, `hobbies`, `interests`, `languages`, `profilePhoto`).
    for (const [k, v] of Object.entries(raw)) {
      if (MATRIMONIAL_STRING_COLS.has(k)) {
        if (typeof v === 'string') data[k] = v;
        else if (v === null || v === undefined) continue;
        else data[k] = String(v);
      } else if (MATRIMONIAL_INT_COLS.has(k)) {
        if (v === null || v === undefined || v === '') continue;
        const n = typeof v === 'number' ? v : parseInt(String(v), 10);
        if (Number.isFinite(n)) data[k] = n;
      } else if (MATRIMONIAL_BOOL_COLS.has(k)) {
        data[k] = v === true || v === 'true' || v === 1 || v === '1';
      ...
      // else: silently drop unknown keys (email, hobbies, interests, etc.)
    }
    ...
    // Recompute completion score after matrimonial write — the completion
    // engine depends on partnerAgeMin/Max + partnerReligion/etc. being present.
    recomputeAndPersistCompletion(prisma, req.userId!).catch(() => {});
    res.json({ data: profile });
  } catch (e) { next(e); }
});
...
// ═══ v3.6.0 — Move v2 suggestions ════════════════════════════
// Returns 5 ranked opener suggestions, scored by sender-voice fingerprint,
// receiver resonance, hook strength, and right-now-intent. The v1
// /move-suggestions route remains untouched and continues to be used when
// the flag is OFF.
app.post(
  '/api/v1/creativity/items/:id/move-suggestions-v2',
  authMiddleware,
  validate({ body: moveSuggestionsV2BodySchema }),
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    if (!isMoveV2Enabled()) { res.status(404).json({ error: { message: 'Not found', code: 'NOT_FOUND' } }); return; }
    try {
      const userId = req.userId!;
      const { id } = req.params;
      const n = Math.min(5, Math.max(1, Number(req.body?.n) || 5));
      const seed = Number.isFinite(Number(req.body?.seed)) ? Number(req.body?.seed) : Math.floor(Math.random() * 0x7fffffff);

      // Resolve receiver = author of the creativity item
      const item = await prisma.creativityItem.findUnique({
        where: { id },
        select: { authorId: true, content: true, author: { select: { id: true, displayName: true, profile: { select: { city: true, datingIntent: true, languages: true } } } } },
      }).catch(() => null);
      if (!item) { res.status(404).json({ error: { message: 'Item not found', code: 'NOT_FOUND' } }); return; }
      const receiverId = item.authorId;

      // 1. Sender voice — last 50 outbound messages
      const senderMsgs = await prisma.message.findMany({
        where: { senderId: userId, type: 'text' },
        orderBy: { createdAt: 'desc' },
        take: 50,
        select: { content: true, createdAt: true },
      }).catch(() => [] as Array<{ content: string; createdAt: Date }>);
      const senderSamples: OutboundMessageSample[] = senderMsgs.map((m) => ({ content: m.content, createdAtMs: m.createdAt.getTime() }));
      const senderVoice = extractSenderVoice(senderSamples);

      // 2. Receiver resonance — last 10 successful FirstMoveOutcome rows for receiver
      const receiverHash = hashUid(receiverId);
      const fmo = await prisma.firstMoveOutcome.findMany({
        where: { bHash: receiverHash, replied: true },
        orderBy: { sentAt: 'desc' },
        take: 10,
        select: { kind: true, replied: true, replyMs: true, meta: true },
      }).catch(() => [] as Array<{ kind: string; replied: boolean; replyMs: number | null; meta: any }>);
      const resonanceSamples: FirstMoveOutcomeSample[] = fmo.map((r) => {
        const m = (r.meta as any) || {};
        const kind = ['question', 'compliment', 'shared_interest', 'playful', 'specific_detail', 'voice_invite', 'photo_invite'].includes(r.kind) ? (r.kind as OpenerKind) : 'question';
        const tone = ['reflective', 'casual', 'tactile', 'quick'].includes(String(m.tone)) ? (String(m.tone) as ResonanceTone) : 'casual';
        return { openerKind: kind, tone, replied: r.replied, replyMs: r.replyMs ?? null, openerLengthChars: Number(m.openerLengthChars ?? 40) };
      });
      const receiverResonance = buildResonance(resonanceSamples);

      // 3. Hooks: hookLibrary + recent posts + shared interests + DTM + festival
      const hooks: HookCandidate[] = [];
      // Recent post hook (the item being commented on)
      if (item.content) {
        hooks.push({ category: 'recent_post', text: String(item.content).slice(0, 80), freshnessAgeDays: 1, specificity: 0.7 });
      }
      // Shared interests
      const [meInterests, themInterests] = await Promise.all([
        prisma.profileInterest.findMany({ where: { userId }, select: { name: true } }).catch(() => []),
        prisma.profileInterest.findMany({ where: { userId: receiverId }, select: { name: true } }).catch(() => []),
      ]);
      const themSet = new Set(themInterests.map((i) => i.name.toLowerCase()));
      for (const mi of meInterests) {
        if (themSet.has(mi.name.toLowerCase())) {
          hooks.push({ category: 'shared_interest', text: mi.name, freshnessAgeDays: Infinity, specificity: 0.5 });
        }
      }
      // DTM agreement (best-effort, only when both have DTM)
      try {
        const [aDtm, bDtm] = await Promise.all([
          prisma.matrimonialProfile.findUnique({ where: { userId } }),
          prisma.matrimonialProfile.findUnique({ where: { userId: receiverId } }),
        ]);
        if (aDtm && bDtm) {
          if ((aDtm as any).religion && (aDtm as any).religion === (bDtm as any).religion) hooks.push({ category: 'dtm_topic', text: 'family values', freshnessAgeDays: Infinity, specificity: 0.4 });
        }
      } catch { /* ignore */ }
      // Festival hooks
      const festivals = festivalHooksForToday(Date.now());
      for (const phrase of festivals.slice(0, 3)) {
        hooks.push({ category: 'festival', text: phrase, freshnessAgeDays: 0, specificity: 0.6 });
      }

      // 4. Language family from sender's last 20 messages
      const senderLangSamples = senderSamples.slice(0, 20).map((s) => s.content);
      const langDetect = detectLanguageFamily(senderLangSamples);
      const languageFamily: LanguageFamily = langDetect.family;
      ...
      const composerInput: ComposerInput = {
        ...
        hooks,
        languageFamily,
        receiverName: item.author?.displayName ?? undefined,
        seed,
        nowMs: Date.now(),
      };
      const result = moveCompose(composerInput);
      const generatedAt = new Date();

      // 6. Emit tracking event — legacy UserActivity row (back-compat) +
      // v8 strict-validated `move.composed` event so the v8 algos
      // (move-suggestion-acceptance KPI, opener style learning) actually
      // receive the signal.
      trackActivity(prisma, userId, 'move', 'composed', id, { fallbackCount: result.fallbackCount, languageFamily, count: result.suggestions.length });
      // Slice to schema cap (5 hookCategories max, dedup-preserving order).
      const hookCategoriesSeen = new Set<string>();
      const hookCategories: string[] = [];
      for (const s of result.suggestions) {
        const c = String(s.hookCategory || '').slice(0, 32);
        if (c && !hookCategoriesSeen.has(c)) {
          hookCategoriesSeen.add(c);
          hookCategories.push(c);
          if (hookCategories.length >= 5) break;
        }
      }
      emitServerEvent(prisma, userId, 'move.composed', {
        receiverHash,
        suggestionCount: Math.min(5, result.suggestions.length),
        fallbackCount: Math.min(5, result.fallbackCount),
        hookCategories,
        languageFamily,
      }, id);

      // Phase C.3 — Prometheus counter, labelled by composer source.
      // Cardinality bounded to {v1, v2}; alarm on v1-fallback rate.
      try { moveV2SuggestionsEmitted.inc({ source: 'v2' }); } catch { /* metrics never block */ }
      res.json({
        suggestions: result.suggestions.slice(0, n),
...
// ═══ v3.6.0 — Family Brief generate + view ════════════════════
// Generates a short, shareable summary of the requester's DTM profile,
// addressable by a 22-char base64url token, valid for 7 days.
function generateBriefToken(): string {
  return crypto.randomBytes(17).toString('base64url').slice(0, 22);
}

function renderFamilyBriefText(args: {
  displayName: string | null;
  profile: { age?: number | null; city?: string | null; profession?: string | null; religion?: string | null; education?: string | null } | null;
  dtm: { familyBackground?: string | null; subCommunity?: string | null; educationLevel?: string | null; employer?: string | null; maritalStatus?: string | null; willingToRelocate?: boolean | null; expectedTimeline?: string | null } | null;
}): string {
  const lines: string[] = [];
  lines.push(`Family Brief — ${args.displayName ?? 'Miamo Member'}`);
  if (args.profile?.age) lines.push(`Age: ${args.profile.age}`);
  if (args.profile?.city) lines.push(`City: ${args.profile.city}`);
  if (args.profile?.profession) lines.push(`Profession: ${args.profile.profession}`);
  if (args.profile?.religion) lines.push(`Religion: ${args.profile.religion}`);
  if (args.profile?.education) lines.push(`Education: ${args.profile.education}`);
  if (args.dtm?.subCommunity) lines.push(`Community: ${args.dtm.subCommunity}`);
  if (args.dtm?.educationLevel) lines.push(`Education level: ${args.dtm.educationLevel}`);
  if (args.dtm?.employer) lines.push(`Employer: ${args.dtm.employer}`);
  if (args.dtm?.maritalStatus) lines.push(`Status: ${args.dtm.maritalStatus}`);
  if (args.dtm?.willingToRelocate != null) lines.push(`Open to relocate: ${args.dtm.willingToRelocate ? 'Yes' : 'No'}`);
  if (args.dtm?.expectedTimeline) lines.push(`Timeline: ${args.dtm.expectedTimeline}`);
  if (args.dtm?.familyBackground) lines.push(`\nFamily: ${args.dtm.familyBackground}`);
  return lines.join('\n');
}

// ═══ DTM next-batch (v8 topic-mask wiring) ══════════════════════════════
//
// Returns the next batch of DTM topics to ask the user. v7's `buildDtmFeed`
// does the ranking; v8 adds a viewer-state mask (mood / late-night / window-
// shopping / coverage-stage) that suppresses heavy topics under the
// conditions defined in `dtmTopicMask.ts`.
//
// The mask is GATED behind `FEATURE_DTM_MASK_ENABLED`. When the env var is
// unset or any value other than '1', the route is byte-identical to a plain
// v7 `buildDtmFeed` call — i.e. shipping this commit with the flag off is a
// no-op (modulo the new endpoint being available at all).
//
// Request body:
//   { candidates: DtmTopicCandidate[], history?: Record<topic, DtmTopicHistory>,
//     weights?: Record<topic, number>, k?: number }
// Header (mask only): X-Local-Hour: 0..23 — caller's local-tz hour. Required
//   for the late-night rule; absent ⇒ UTC hour 12 ⇒ never triggers late-night
//   (cheap default, never mid-rule).
app.post('/api/v1/dtm/next-batch', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const userId = req.userId!;
    const body = req.body ?? {};
    const candidates = Array.isArray(body.candidates) ? body.candidates as DtmTopicCandidate[] : [];
    if (candidates.length === 0) {
      res.status(400).json({ error: { message: 'candidates required', code: 'VALIDATION_ERROR' } });
      return;
    }
    const k = Number.isFinite(Number(body.k)) ? Number(body.k) : 10;
    const weights = new Map<string, number>(
      body.weights && typeof body.weights === 'object'
        ? Object.entries(body.weights as Record<string, unknown>).map(([t, v]) => [t, Number(v) || 0] as [string, number])
        : [],
    );
    const history = new Map<string, DtmTopicHistory>(
      body.history && typeof body.history === 'object'
        ? Object.entries(body.history as Record<string, unknown>).map(([t, v]) => {
            const h = v as Partial<DtmTopicHistory> | null | undefined;
            return [t, {
              answered: Number(h?.answered) || 0,
              skippedRecently: h?.skippedRecently === true,
              abandonedRecently: h?.abandonedRecently === true,
            }] as [string, DtmTopicHistory];
          })
        : [],
    );

    // FAST PATH — flag OFF. Byte-identical to the v7 ranker.
    if (!isDtmMaskEnabled()) {
      const result = buildDtmFeed({ weights, candidates, history, k });
      res.json({ batch: result.batch, rejected: result.rejected, maskReason: 'flag_off' });
      return;
    }

    // SLOW PATH — flag ON. Compute mask inputs from FeatureSnapshot + SessionSummary + coverage.
    const uidHash = hashUid(userId);
    const nowMs = Date.now();

    // 1) moodGuess — from FeatureSnapshot.raw.moodRightNow if present & fresh.
    // Stale or missing ⇒ neutral 0.5 (no mask trigger via the mood rule).
    let moodGuess = 0.5;
    try {
      const snap = await prisma.featureSnapshot.findUnique({ where: { uidHash } });
      const raw = (snap as any)?.raw as { moodRightNow?: { fatigued?: number; computedAt?: number; ttlMs?: number } } | undefined;
      const m = raw?.moodRightNow;
      if (m && typeof m.fatigued === 'number') {
        const computedAt = typeof m.computedAt === 'number' ? m.computedAt : 0;
        const ttlMs = typeof m.ttlMs === 'number' ? m.ttlMs : MOOD_TTL_MS;
        const fresh = computedAt > 0 && computedAt + ttlMs > nowMs;
        if (fresh) {
          // moodGuess = 1 - fatigued — a tired user has a "low mood" proxy.
          moodGuess = Math.max(0, Math.min(1, 1 - m.fatigued));
        }
      }
    } catch {
      // Degrade gracefully — leave moodGuess at neutral 0.5.
    }

    // 2) Last 2 SessionSummary rows for window-shopping streak.
    let recentSessionFlags: Array<{ windowShopping: boolean; ghostedSelf: boolean }> = [];
    try {
      const sessions = await prisma.sessionSummary.findMany({
        where: { uidHash },
        orderBy: { endedAt: 'desc' },
        take: 2,
        select: { windowShopping: true, ghostedSelf: true },
      });
      recentSessionFlags = sessions.map((s) => ({ windowShopping: !!s.windowShopping, ghostedSelf: !!s.ghostedSelf }));
    } catch {
      recentSessionFlags = [];
    }

    // 3) Coverage stage — count distinct topics user has answered (UserActivity
    // dtm_answer rows). The pure helper accepts a Float32Array/number[] sized
    // to DTM_TOPIC_COUNT; we synthesise it from distinct answered topics.
    let coverageStage: 'empty' | 'sparse' | 'sufficient' | 'full' = 'empty';
    try {
      const answers = await prisma.userActivity.findMany({
        where: { userId, action: 'dtm_answer' },
        select: { targetId: true },
        take: 200,
      });
      const vec = new Float32Array(DTM_TOPIC_KEYS.length);
      const seen = new Set<string>();
      for (const a of answers) {
        const t = (a.targetId ?? '').trim();
        if (!t || seen.has(t)) continue;
        const idx = DTM_TOPIC_KEYS.indexOf(t as DtmTopicKey);
        if (idx >= 0) { vec[idx] = 1; seen.add(t); }
      }
      coverageStage = dtmColdStart(vec).stage;
    } catch {
      coverageStage = 'empty';
    }

    // 4) localHour from header, fall back to UTC 12 (never triggers late-night).
    const rawHour = req.header('x-local-hour');
    let localHour: number | null = null;
    if (rawHour !== undefined && rawHour !== null && String(rawHour).trim() !== '') {
      const n = Number(rawHour);
      if (Number.isInteger(n) && n >= 0 && n <= 23) localHour = n;
    }
    if (localHour === null) localHour = 12; // because: prompt §3 — UTC 12 default ⇒ no mask trigger

    const { result, mask, effectiveMask } = buildMaskedDtmFeed({
      feed: { weights, candidates, history, k },
      mask: { moodGuess, recentSessionFlags, coverageStage, localHour },
    });

    // Emit `dtm.topic_masked` per spec §3: when reason !== 'no_mask' AND a chosen
    // batch topic was originally in blockedTopics. Under standard mask semantics
    // this never fires (blocked topics are filtered BEFORE ranking) — but we
    // ALSO emit one event per topic the mask rejected, which is the metric the
    // dashboard actually consumes ("how many topics did the mask drop today?").
    if (mask.reason !== 'no_mask') {
      const dropped = maskRejectedTopics(result);
      for (const t of dropped) {
        trackActivity(prisma, userId, 'dtm.topic_masked', 'dtm_topic', t, { reason: mask.reason });
      }
    }
    ...
    res.json({
      ...
      allowedTopics: effectiveMask,
      blockedTopics: mask.blockedTopics,
    });
  } catch (e) { next(e); }
});
...
app.post(
  ...
  authMiddleware,
  idempotency(),
  validate({ body: familyBriefGenerateBodySchema }),
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    if (!isFamilyBriefEnabled()) { res.status(404).json({ error: { message: 'Not found', code: 'NOT_FOUND' } }); return; }
    try {
      const userId = req.userId!;
      const format = String(req.body?.format) as 'pdf' | 'image' | 'text';
      const trackViews = req.body?.trackViews === true;
      const token = generateBriefToken();
      const now = new Date();
      const expiresAt = new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000);

      await prisma.familyBriefShare.create({
        data: { userId, token, format, generatedAt: now, expiresAt, viewCount: 0, trackViews },
      });

      const url = `/api/v1/dtm/family-brief/${token}`;
      trackActivity(prisma, userId, 'family_brief', 'generated', token, { format, trackViews });
      res.json({ token, url, expiresAt: expiresAt.toISOString(), note: format === 'text' ? 'text content available via GET /api/v1/dtm/family-brief/:token' : 'PDF/image rendering deferred; text fallback available via GET' });
    } catch (e) { next(e); }
  },
);

// Phase F — GET /api/v1/dtm/family-brief/shares
// Lists the caller's own family-brief share history so the DTM UI can show
// a "your briefs" dashboard (view counts, expiry, format). Rate-limited via
// the gateway's authenticated-tier limiter (30 req/min/user). Behind
// FEATURE_FAMILY_BRIEF_SHARES_ENABLED so ops can dark-launch. Returns only
// the caller's own rows — never other users' tokens.
app.get('/api/v1/dtm/family-brief/shares', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  if (!isFamilyBriefEnabled() || process.env.FEATURE_FAMILY_BRIEF_SHARES_ENABLED !== '1') {
  ...
  try {
    const userId = req.userId!;
    const now = Date.now();
    const rows = await prisma.familyBriefShare.findMany({
      where: { userId },
      orderBy: { generatedAt: 'desc' },
      take: 50,
      select: { id: true, token: true, format: true, generatedAt: true, expiresAt: true, viewCount: true, trackViews: true },
    });
    const shaped = rows.map((r) => ({
      ...
      trackViews: r.trackViews,
      expired: r.expiresAt.getTime() < now,
      url: `/api/v1/dtm/family-brief/${r.token}`,
    }));
    const summary = {
      total: shaped.length,
      active: shaped.filter((r) => !r.expired).length,
      totalViews: shaped.reduce((acc, r) => acc + (r.viewCount || 0), 0),
    };
    res.json({ data: shaped, summary });
  } catch (e) { next(e); }
});

// PUBLIC route — no authMiddleware. Gateway must also avoid auth on this path.
app.get('/api/v1/dtm/family-brief/:token', async (req: Request, res: Response, next: NextFunction) => {
  if (!isFamilyBriefEnabled()) { res.status(404).json({ error: { message: 'Not found', code: 'NOT_FOUND' } }); return; }
  try {
    const { token } = req.params;
    if (!token || typeof token !== 'string' || token.length < 8 || token.length > 64) { res.status(400).json({ error: { message: 'Invalid token', code: 'VALIDATION_ERROR' } }); return; }
    const share = await prisma.familyBriefShare.findUnique({ where: { token } });
    if (!share) { res.status(404).json({ error: { message: 'Not found', code: 'NOT_FOUND' } }); return; }
    if (share.expiresAt.getTime() < Date.now()) { res.status(404).json({ error: { message: 'Expired', code: 'NOT_FOUND' } }); return; }
    const [user, dtm] = await Promise.all([
      prisma.user.findUnique({ where: { id: share.userId }, include: { profile: true } }),
      prisma.matrimonialProfile.findUnique({ where: { userId: share.userId } }),
    ]);
    const text = renderFamilyBriefText({
      displayName: user?.displayName ?? null,
      profile: user?.profile ? {
        age: user.profile.age, city: user.profile.city, profession: user.profile.profession, religion: user.profile.religion, education: user.profile.education,
      } : null,
      dtm: dtm ? {
        familyBackground: (dtm as any).familyBackground ?? null,
        subCommunity: (dtm as any).subCommunity ?? null,
        educationLevel: (dtm as any).educationLevel ?? null,
        employer: (dtm as any).employer ?? null,
        maritalStatus: (dtm as any).maritalStatus ?? null,
        willingToRelocate: (dtm as any).willingToRelocate ?? null,
        expectedTimeline: (dtm as any).expectedTimeline ?? null,
      } : null,
    });
    if (share.trackViews) {
      await prisma.familyBriefShare.update({ where: { token }, data: { viewCount: { increment: 1 } } }).catch(() => null);
      trackActivity(prisma, share.userId, 'family_brief', 'viewed', token, { format: share.format });
    }
    res.json({ token, format: share.format, expiresAt: share.expiresAt.toISOString(), text, displayName: user?.displayName ?? null });
  } catch (e) { next(e); }
});

// ─── Payments (v1.1 stubs) ──────────────────────────
// These routes are placeholders so the middleware (idempotency + rate-limit)
// is wired today. Real Razorpay integration ships in B.4. Today they return
// 501 NOT_IMPLEMENTED, but mounting the limiters means an attacker that
// stumbles on the route still gets throttled.
//
// IMPORTANT: the webhook does NOT use `idempotency()` middleware — provider
// signatures (Razorpay-Signature HMAC) are the canonical replay gate. Adding
// our generic Idempotency-Key check on top would require the provider to send
// that header, which they don't. The handler itself (when implemented) must
// reject duplicate `razorpay_payment_id` by unique-constraint on the ledger.
app.post('/api/v1/payments/spotlight/order', authMiddleware, paymentInitLimiter, idempotency(), async (_req: AuthRequest, res: Response) => {
  res.status(501).json({ error: { code: 'NOT_IMPLEMENTED', message: 'Payment integration ships in v1.1.', statusCode: 501 } });
});

app.post('/api/v1/payments/spotlight/verify', authMiddleware, paymentInitLimiter, idempotency(), async (_req: AuthRequest, res: Response) => {
  res.status(501).json({ error: { code: 'NOT_IMPLEMENTED', message: 'Payment integration ships in v1.1.', statusCode: 501 } });
});

// Webhook: NO authMiddleware (gateway must allowlist this path), NO idempotency()
// (provider signature is the replay gate). Rate-limit only — providers can
// burst-retry on transient 5xx.
app.post('/api/v1/payments/webhook', webhookLimiter, async (_req: Request, res: Response) => {
  res.status(501).json({ error: { code: 'NOT_IMPLEMENTED', message: 'Payment integration ships in v1.1.', statusCode: 501 } });
});

// ─── v1.2 Task 3a — DTM Mutual Interest → Match ─────────────────
// POST /api/v1/dtm/mutual-interest — atomically records a one-sided
// DtmInterest and, if the target had previously recorded an interest
// toward the requester, creates the DtmMatch + notifications inside the
// same $transaction. Flag-gated behind `FEATURE_DTM_MATCH_ENABLED=1`
// (default OFF; returns 404 when off so unwired UI can't accidentally
// spam the endpoint during rollout).
function isDtmMatchEnabled(): boolean {
  return process.env.FEATURE_DTM_MATCH_ENABLED === '1';
}
app.post('/api/v1/dtm/mutual-interest', authMiddleware, idempotency(), validate({ body: dtmMutualInterestBodySchema }), async (req: AuthRequest, res: Response, next: NextFunction) => {
  if (!isDtmMatchEnabled()) { res.status(404).json({ error: { message: 'Not found', code: 'NOT_FOUND' } }); return; }
  try {
    const fromUserId = req.userId!;
    const { targetUserId } = req.body as { targetUserId: string };
    if (targetUserId === fromUserId) {
      res.status(400).json({ error: { message: 'Cannot DTM-match yourself', code: 'VALIDATION_ERROR' } });
      return;
    }
    // Everything atomic: interest upsert + mutual check + match create +
    // notifications. A concurrent request from the target arriving between
    // the interest upsert and the match check would race — the $transaction
    // + the DtmMatch unique index (user1Id, user2Id) resolves the race so
    // exactly one match row survives.
    const result = await (prisma as any).$transaction(async (tx: any) => {
      // Interest upsert (idempotent — same requester expressing interest
      ...
      // Did the target already express interest toward us? If yes → match.
      ...
      // Canonicalise (user1Id, user2Id) ordering so the unique index dedupes
      // regardless of which side commits first.
      ...
      await tx.notification.create({ data: { userId: targetUserId, type: 'dtm_match', title: 'A serious match on DTM 💍', body: 'Someone shares your intent to marry. Say hi.' } });
      await tx.notification.create({ data: { userId: fromUserId,  type: 'dtm_match', title: 'A serious match on DTM 💍', body: 'Someone shares your intent to marry. Say hi.' } });
      return { matched: true, match: created };
    }).catch((e: any) => { logger.warn('dtm/mutual-interest txn failed', e); return { matched: false, match: null }; });

    res.json({ data: { matched: result.matched, match: result.match, source: 'dtm' as const } });
  } catch (e) { next(e); }
});

// ─── v1.2 Task 3c — Admin Fairness Gini ─────────────────────────
// GET /api/v1/admin/fairness-gini — returns the last-week's fairness
// audit rows (produced by tracking-worker/fairnessAudit.ts) keyed by
// gender bucket. Requires `User.isAdmin=true`. Flag-gated behind
// `FEATURE_ADMIN_FAIRNESS_ENABLED=1` (default OFF).
function isAdminFairnessEnabled(): boolean {
  return process.env.FEATURE_ADMIN_FAIRNESS_ENABLED === '1';
}
async function requireAdmin(req: AuthRequest, res: Response): Promise<boolean> {
  const userId = req.userId;
  if (!userId) { res.status(401).json({ error: { message: 'Unauthenticated', code: 'UNAUTHENTICATED' } }); return false; }
  const u = await (prisma as any).user.findUnique({ where: { id: userId }, select: { isAdmin: true } }).catch(() => null);
  if (!u?.isAdmin) { res.status(403).json({ error: { message: 'Admin only', code: 'FORBIDDEN' } }); return false; }
  return true;
}
app.get('/api/v1/admin/fairness-gini', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  if (!isAdminFairnessEnabled()) { res.status(404).json({ error: { message: 'Not found', code: 'NOT_FOUND' } }); return; }
  try {
    if (!(await requireAdmin(req, res))) return;
    // Read the last week's audit rows from AuditLog (written by
    // tracking-worker/fairnessAudit.ts with action='fairness_audit').
    const since = new Date(Date.now() - 7 * 86400_000);
    const rows = await (prisma as any).auditLog.findMany({
      where: { action: 'fairness_audit', createdAt: { gte: since } },
      orderBy: { createdAt: 'desc' },
      take: 200,
      select: { id: true, createdAt: true, details: true },
    }).catch(() => [] as Array<{ id: string; createdAt: Date; details: unknown }>);
    // Parse the JSON details column into { gini: {m,f,o}, threshold, overBuckets, computedAt, usersAudited }.
    const audits = rows.map((r: any) => {
      let parsed: any = null;
      try { parsed = typeof r.details === 'string' ? JSON.parse(r.details) : r.details; } catch { parsed = null; }
      return {
        id: r.id,
        createdAt: r.createdAt,
        gini: parsed?.gini ?? null,
        threshold: parsed?.threshold ?? null,
        overBuckets: parsed?.overBuckets ?? [],
        usersAudited: parsed?.usersAudited ?? null,
      };
    });
    const latest = audits[0] ?? null;
    res.json({ data: { latest, history: audits, lookbackDays: 7 } });
  } catch (e) { next(e); }
});
...
if (process.env.NODE_ENV !== 'test') {
  ...
  // Mount Spotlight (creativity v2) routes and start the in-process trending worker.
  mountCreativitySpotlight(app, prisma, authMiddleware);
  startTrendingWorker(prisma);
