...
app.get('/api/v1/users', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    // /users list is publicly listable to any authed user — must strip PII.
    res.json({ data: users.map(u => toPublicUser(u as any)) });
  } catch (e) { next(e); }
});
...
app.get('/api/v1/users/me', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    // Own user — keep email/phone/2FA state etc (client uses them for own settings)
...
app.get('/api/v1/users/:id', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (!user) return res.status(404).json({ error: { message: 'User not found' } });
    // Someone ELSE's user row — must not leak email/phone/googleId/isAdmin/etc.
    // Requester's own row is served by /users/me above (which keeps PII).
    if (user.id === req.userId) {
      const { passwordHash, ...rest } = user;
      return res.json({ data: rest });
    }
    res.json({ data: toPublicUser(user as any) });
  } catch (e) { next(e); }
});
  ...
  try {
    ...
    if (['all', 'user', 'name', 'city', 'id'].includes(searchType)) {
      ...
      const scored = users.map(u => {
        // Search results = other users → strip PII (email, phone, etc.)
        const rest = toPublicUser(u as any);
