...
import { markReadBodySchema, internalNotificationBodySchema, internalNotificationScheduleBodySchema, notificationRegisterDeviceBodySchema } from '../../shared/src/schemas';
...
const sentry = installSentry({ serviceName: 'notifications' });
...
// Guard for /internal/* routes. Runs BEFORE Zod so we return 403 on a
// wrong key regardless of body shape (i.e. don't leak whether the body was
// well-formed to anonymous callers).
function requireInternalKey(req: Request, res: Response, next: NextFunction) {
  // bug-hunt part2 fix #2 — constant-time key compare.
  if (!timingSafeStringEqual(req.headers['x-internal-key'], env.internalServiceKey)) {
...
app.post('/api/v1/notifications/mark-read', authMiddleware, idempotency(), validate({ body: markReadBodySchema }), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { ids } = req.body;
...
app.post('/internal/notifications', requireInternalKey, validate({ body: internalNotificationBodySchema }), async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // The Notification.data column is String — normalize object input to JSON now
    // so both legacy (string body) and new (object body) callers persist correctly.
    let finalData: string = typeof data === 'string' ? data : (data ? JSON.stringify(data) : '{}');
    if (v4RankEnabled('notifications')) {
      try {
        ...
        const parsed = (() => { try { return JSON.parse(finalData); } catch { return {}; } })();
...
app.post('/internal/notifications/schedule', requireInternalKey, validate({ body: internalNotificationScheduleBodySchema }), async (req: Request, res: Response, next: NextFunction) => {
  try {
    if (!v4RankEnabled('notifications')) return res.status(404).json({ error: { message: 'v4 notifications disabled' } });
    const { userId, minSpacingSec = 1800, tzOffsetMin = 0, quietHours = null } = req.body as {
      userId: string;
...
// The Expo mobile app calls this on cold start after obtaining a push token.
// Idempotent upsert on `token` so re-installs and token rotations don't
// create duplicate rows. The notifications worker fans out via Expo's push
// API using the persisted tokens.
app.post(
  '/api/v1/notifications/register-device',
  authMiddleware,
  validate({ body: notificationRegisterDeviceBodySchema }),
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      const { platform, token } = req.body as { platform: 'ios' | 'android'; token: string };
      await (prisma as any).notificationDevice.upsert({
        where: { token },
        create: {
          userId: req.userId!,
          platform,
      ...
      logger.error({ err }, 'register-device failed');
      return next(err);
    }
  },
);
