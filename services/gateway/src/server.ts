...
app.use(requestId);
// Sentry request handler — mounts after metrics + requestId so trace
// correlation works, but before helmet/cors/routes so every request is
// captured. No-op when SENTRY_DSN is unset.
const sentry = installSentry({ serviceName: 'gateway' });
...
// bug-hunt part2 fix #2 — was `key !== INTERNAL_KEY` (V8 short-circuit
// timing side-channel). Now uses timingSafeStringEqual across all 7 sites.
app.post('/internal/push-event', express.json({ limit: '1mb' }), (req: express.Request, res: express.Response) => {
  const key = req.headers['x-internal-key'];
  if (!timingSafeStringEqual(key, INTERNAL_KEY)) {
...
// bug-hunt part2 fix #7 companion (docs/architecture/bug-hunt-2026-07-part2.md
// #8) — previous implementation fire-and-forgot with no timeout; a hung
// social service leaked gateway sockets on every activity ping. Now bounded
// by AbortSignal.timeout(2000) and the requestId is forwarded so downstream
// logs correlate.
app.post('/api/v1/activity/track', express.json({ limit: '50kb' }), (req: express.Request, res: express.Response) => {
  ...
  const headers: Record<string, string> = { 'Content-Type': 'application/json', 'x-user-id': userId, 'x-internal-key': INTERNAL_KEY };
  const rid = req.headers['x-request-id'];
  if (typeof rid === 'string' && rid.length <= 128) headers['x-request-id'] = rid;
...
// ─── v3.6.0 routes ───
// Weekly Top-10 → social
app.use('/api/v1/weekly-top', requireAuth, createProxyMiddleware(proxyTo(SERVICES.social)));
// Family Brief share viewer is PUBLIC (no auth) so a non-Miamo family member
// can open the share link. The generate path stays auth-gated implicitly via
// the content service auth middleware on the POST route.
app.use(/^\/api\/v1\/dtm\/family-brief\/[^/]+$/, createProxyMiddleware(proxyTo(SERVICES.content)));
app.use('/api/v1/dtm/family-brief', requireAuth, createProxyMiddleware(proxyTo(SERVICES.content)));
// (Move v2 suggestions live under /api/v1/creativity/... already proxied to content above.
// Why-explainer lives under /api/v1/discover/... already proxied to social above.)

// Notification routes (protected)
app.use('/api/v1/notifications', requireAuth, createProxyMiddleware(proxyTo(SERVICES.notifications)));

// Payments (v1.1 — currently stub 501s, but the proxy needs to exist so the
// per-endpoint paymentInitLimiter / webhookLimiter at the content service
// actually fire). Webhook is PUBLIC (no auth) — provider signature is gated
// inside the handler.
app.use(/^\/api\/v1\/payments\/webhook$/, createProxyMiddleware(proxyTo(SERVICES.content)));
app.use('/api/v1/payments', requireAuth, createProxyMiddleware(proxyTo(SERVICES.content)));

// ─── Sentry error handler ────────────────────────────
// Catches errors bubbling up from any proxy/route handler above. Reports to
// Sentry then forwards via next(err) so Express's default 500 envelope still
// fires. No-op when SENTRY_DSN is unset.
