...
// ─── Phase G.11 — Text moderation flag ─────────────
// Default OFF via FEATURE_TEXT_MODERATION_ENABLED. When ON, we run message
// text through the shared moderator before persisting. Hard-severity hits
// are audit-logged + rejected; soft hits ask the user to re-edit.
...
app.get('/api/v1/messages/chats', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    for (const c of chats) {
      ...
      const otherUser = toPublicUser(rawOther as any);
...
app.get('/api/v1/messages/chats/archived', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const result = chats.map(c => {
      ...
      const otherUser = toPublicUser(o as any);
      const lastMsg = c.messages[0] || null;
      const lastContent = lastMsg ? decryptMessage(lastMsg.content) : null;
      const previewContent = previewFromContent(lastContent);
      return {
...
app.post('/api/v1/messages/chats/:chatId/messages', authMiddleware, idempotency(), validate({ body: sendMessageBodySchema }), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    // Default OFF. On hard hits: audit-log + 422. On soft hits: 400.
    // We reject BEFORE the anti-ghost deposit runs so bad first-messages
    // don't burn Spotlight minutes.
    if (isTextModerationEnabled()) {
      const decision = await getTextModerator().moderateText(content);
      if (!decision.approved) {
        if (decision.severity === 'hard') {
          await auditLog(prisma, userId, 'moderation.message.hard_block', {
            chatId,
        ...
        return res.status(400).json({
          error: {
            ...
            message: 'Please edit your message before sending.',
            categories: decision.categories,
            severity: 'soft',
          },
        });
...
app.get('/api/v1/beats', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const result = beats.map(b => {
      ...
      const other = toPublicUser(o as any);
