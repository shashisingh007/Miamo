...
import {
  ...
  loginOtpStartBodySchema,
  loginOtpVerifyBodySchema,
  signupOtpCompleteBodySchema,
...
function authMiddleware(req: AuthRequest, res: Response, next: NextFunction) {
  ...
  if (internalUserId && timingSafeStringEqual(req.headers['x-internal-key'], env.internalServiceKey)) {
...
// v3.7 signup/signin rewrite — resolve whatever the client sent
// (`identifier`, `email`, or `phone`) into a (channel, normalized-value)
// pair. Callers still validate presence via Zod refine; this only picks
// the right channel + normalises the raw string.
type IdentifierBundle = { channel: 'email' | 'phone'; identifier: string };
function resolveIdentifier(body: {
  identifier?: string;
  email?: string;
  phone?: string;
}): IdentifierBundle {
  // Precedence: explicit `phone` > explicit `email` > `identifier` autodetect.
  if (body.phone) {
    const identifier = normalizeIdentifier('phone', String(body.phone));
    if (!isValidPhone(identifier)) throw new AppError('invalid phone (use E.164)', 400, 'VALIDATION_ERROR');
    return { channel: 'phone', identifier };
  }
  if (body.email) {
    const identifier = normalizeIdentifier('email', String(body.email));
    if (!isValidEmail(identifier)) throw new AppError('invalid email', 400, 'VALIDATION_ERROR');
    return { channel: 'email', identifier };
  }
  const raw = String(body.identifier || '').trim();
  if (!raw) throw new AppError('identifier required', 400, 'VALIDATION_ERROR');
  const channel: 'email' | 'phone' = isValidEmail(raw) ? 'email' : 'phone';
  const identifier = normalizeIdentifier(channel, raw);
...
async function verifyAppleIdToken(idToken: string, userPayload?: { email?: string; name?: { firstName?: string; lastName?: string } }): Promise<{ sub: string; email: string; name?: string; email_verified?: boolean }> {
  ...
  // bug-hunt part2 fix #13 (docs/architecture/bug-hunt-2026-07-part2.md #24) —
  // an Apple JWKS outage previously surfaced as a silent 500 with no signal.
  // Log-and-rethrow so Sentry + Prom see it before user complaints do.
  let payload: any;
  try {
    ...
    logger.warn('[oauth.apple.jwks_error]', (e as Error).message);
    throw new AppError('Apple sign-in verification failed', 401, 'OAUTH_APPLE_JWKS_ERROR');
  }
...
app.post('/api/v1/auth/otp/start', otpSendLimiter, idempotency(), async (req: Request, res: Response, next: NextFunction) => {
  try {
    // bug-hunt part2 fix #8 (docs/architecture/bug-hunt-2026-07-part2.md #10)
    // — cap raw input at RFC5321 254-char email limit BEFORE sanitize +
    // regex, so a 10 MB payload doesn't burn CPU on strip-and-normalize.
    const rawSource = String(req.body?.identifier || '');
    if (rawSource.length > 254) throw new AppError('identifier too long', 400, 'VALIDATION_ERROR');
    const rawIdentifier = sanitize(rawSource);
    if (!rawIdentifier) throw new AppError('identifier required', 400, 'VALIDATION_ERROR');
    const channel: 'email' | 'phone' = isValidEmail(rawIdentifier) ? 'email' : 'phone';
    const identifier = normalizeIdentifier(channel, rawIdentifier);
    if (channel === 'email' && !isValidEmail(identifier)) throw new AppError('invalid email', 400, 'VALIDATION_ERROR');
...
// Register
// v3.7 — accepts EITHER `email` OR `phone` OR `identifier` (auto-detected).
// When only a phone is supplied we synthesise a placeholder email keyed on
// the digits so the User.email @unique constraint is still satisfied. The
// real address remains `phone` (also @unique). Password + displayName gate
// stays the same.
app.post('/api/v1/auth/register', accountCreationLimiter, idempotency(), validate({ body: registerBodySchema }), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { password, displayName: rawDisplayName } = req.body;
    const displayName = sanitize(rawDisplayName);
    const { channel, identifier } = resolveIdentifier(req.body);

    // Uniqueness check on whichever column the incoming channel maps to.
    const existing = channel === 'email'
      ? await prisma.user.findUnique({ where: { email: identifier } })
      : await prisma.user.findUnique({ where: { phone: identifier } });
    if (existing) throw new AppError(channel === 'email' ? 'Email already registered' : 'Phone already registered', 409, channel === 'email' ? 'EMAIL_EXISTS' : 'PHONE_EXISTS');
    ...
    // Auto-generate a username from the identifier prefix + random suffix.
    // For phone-only signups we key the synthetic email on the digits so
    // uniqueness still holds and the username derivation is stable.
    const baseEmail = channel === 'email'
      ? identifier
      : `${identifier.replace(/[^0-9]/g, '')}@phone.miamo.local`;
    const username = baseEmail.split('@')[0].toLowerCase().replace(/[^a-z0-9]/g, '') + Math.floor(Math.random() * 100);
    ...
    const tokens = generateTokens(user.id);
    const session = await createSession(user.id, req);
    auditLog(prisma, user.id, 'register', { channel, identifier });

    // Trust the device used to register so they don't get a 2FA prompt
    // immediately on first login. The just-provided channel is still
    // unverified — issue a verification OTP synchronously.
    const ctx = reqCtx(req);
    const fp = deviceFingerprint(ctx);
    await trustDevice(prisma, user.id, fp, { ip: ctx.ip, label: deviceLabel(ctx) });
    try {
    ...
    } catch (e) { logger.warn('post-register verify OTP issue failed', e); }
    ...
    res.status(201).json({
      data: {
        ...
        verification: channel === 'email'
          ? { emailOtpSentTo: maskIdentifier('email', identifier) }
          : { phoneOtpSentTo: maskIdentifier('phone', identifier) },
      },
    });
  } catch (e) { next(e); }
});
...
// Login
// v3.7 — accepts EITHER `email` OR `phone` OR `identifier` (auto-detected).
// Everything downstream (bcrypt compare, 2FA challenge, session issue) is
// identical to the legacy shape — we just look the User up by whichever
// column the incoming channel maps to.
app.post('/api/v1/auth/login', loginAttemptLimiter, validate({ body: loginBodySchema }), async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const user = channel === 'email'
      ? await prisma.user.findUnique({ where: { email: identifier }, include: { profile: true, photos: { orderBy: { position: 'asc' }, take: 1 } } })
      : await prisma.user.findUnique({ where: { phone: identifier }, include: { profile: true, photos: { orderBy: { position: 'asc' }, take: 1 } } });
    // Timing attack mitigation: always run bcrypt even if user not found
    // This ensures consistent response time regardless of identifier validity.
    if (!user) {
    ...
    const tokens = generateTokens(user.id);
    const session = await createSession(user.id, req);
    auditLog(prisma, user.id, 'login', { channel, identifier });
    setRefreshCookie(res, tokens.refreshToken);
    res.json({
      data: {
        user: {
...
// ─── Passwordless login for existing accounts ────────
// Distinct from /otp/start which auto-creates on unknown identifiers.
// Here we require the user to already exist. To avoid account-enumeration
// the response shape is identical whether we found the user or not — we
// just silently no-op the OTP issue when there's no match. A challenge
// token bound to the user + device fingerprint is returned so verify can
// mint session tokens without a second identifier round-trip.
app.post('/api/v1/auth/login/otp/start', otpSendLimiter, idempotency(), validate({ body: loginOtpStartBodySchema }), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { channel, identifier } = resolveIdentifier(req.body);
    const ctx = reqCtx(req);
    const fp = deviceFingerprint(ctx);
    ...
    // Shape-preserving no-op when the identifier isn't associated with an
    // account. We still return a syntactically-valid challenge token so
    // the client's verify-step contract stays uniform; the token binds a
    // random user id that will never match, so verify will fail with an
    // opaque "invalid code" downstream.
    if (!user) {
      const fakeUserId = randomBytes(16).toString('hex');
      const token = issueChallengeToken(fakeUserId, fp);
      // ~10 minutes from now so the response is indistinguishable from
      // the real path (same TTL as issueOtp() sets).
    ...
    try {
      ...
      const token = issueChallengeToken(user.id, fp);
      auditLog(prisma, user.id, 'login_otp_challenge', { channel });
...
app.post('/api/v1/auth/login/otp/verify', idempotency(), validate({ body: loginOtpVerifyBodySchema }), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { otpToken, code } = req.body as { otpToken: string; code: string };
    const ctx = reqCtx(req);
    const fp = deviceFingerprint(ctx);
    let userId: string;
    try { ({ userId } = verifyChallengeToken(otpToken, fp)); }
    catch (e) { if (otpHandler(e, res)) return; throw e; }

    const user = await prisma.user.findUnique({ where: { id: userId }, include: { profile: true, photos: { orderBy: { position: 'asc' }, take: 1 } } });
    if (!user) throw new AppError('Invalid code', 400, 'OTP_INVALID');
    if (!user.active || user.deactivated) throw new AppError('Account deactivated', 403, 'ACCOUNT_DEACTIVATED');

    // Prefer phone channel if the user has a verified phone; otherwise
    // email. This mirrors the 2FA path and lets a phone-only user log
    // in via SMS OTP.
    const channel: 'email' | 'phone' = user.phone && (user.phoneVerified || !user.emailVerified) ? 'phone' : 'email';
    const identifier = channel === 'phone' ? user.phone! : user.email;
    try { await verifyOtp({ prisma, channel, identifier, purpose: 'login_2fa', code }); }
    catch (e) { if (otpHandler(e, res)) return; throw e; }

    // First successful login via OTP on this channel implicitly verifies it.
    if ((channel === 'email' && !user.emailVerified) || (channel === 'phone' && !user.phoneVerified)) {
    ...
    const tokens = generateTokens(user.id);
    const session = await createSession(user.id, req);
    auditLog(prisma, user.id, 'login_otp_success', { channel });
    setRefreshCookie(res, tokens.refreshToken);
    res.json({
      data: {
        user: {
...
// ─── OTP-only signup complete ────────────────────────
// For users who chose "Verify with OTP" during signup and haven't set a
// password. Reuses the same verifiedToken flow as /signup/complete; the
// difference is we mint a random unusable passwordHash and mark the
// verified channel. The user can set a real password later via
// forgot-password.
app.post('/api/v1/auth/signup/otp-complete', accountCreationLimiter, idempotency(), validate({ body: signupOtpCompleteBodySchema }), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { verifiedToken, displayName: rawDisplayName } = req.body as { verifiedToken: string; displayName: string };
    const { channel, identifier } = verifySignupVerifiedToken(verifiedToken);
    const displayName = sanitize(rawDisplayName);
    // Race-safety re-check: another request may have grabbed this
    // identifier between /signup/verify and now.
    const taken = channel === 'email'
      ? await prisma.user.findUnique({ where: { email: identifier } })
      : await prisma.user.findUnique({ where: { phone: identifier } });
    if (taken) throw new AppError(channel === 'email' ? 'Email already registered' : 'Phone already registered', 409, channel === 'email' ? 'EMAIL_EXISTS' : 'PHONE_EXISTS');
    // Random 32-byte hex string — closes the password login path for
    // this account until the user explicitly sets one.
    ...
    const tokens = generateTokens(user.id);
    const session = await createSession(user.id, req);
    auditLog(prisma, user.id, 'signup_otp_only', { channel });
    const ctx = reqCtx(req);
    const fp = deviceFingerprint(ctx);
    await trustDevice(prisma, user.id, fp, { ip: ctx.ip, label: deviceLabel(ctx) });
    setRefreshCookie(res, tokens.refreshToken);
