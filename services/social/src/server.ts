...
app.get('/api/v1/discover', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    let v8MoBlock: { applied: boolean; intentClass: string | null; top10Unlocked: boolean; isPremium: boolean } = { applied: false, intentClass: null, top10Unlocked: false, isPremium: false };
    if (isV8DiscoverRankerEnabled() && scoredAll.length > 0) {
      try {
        ...
        v8MoBlock = { applied: true, intentClass: intentRightNow?.topClass ?? null, top10Unlocked: requesterTop10Unlocked, isPremium: requesterIsPremium };
      } catch (err) {
    ...
    if (v8MoBlock.applied) meta.v8 = { multiObjective: true, intentClass: v8MoBlock.intentClass, top10Unlocked: v8MoBlock.top10Unlocked, isPremium: v8MoBlock.isPremium };
...
app.get('/api/v1/weekly-top', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (!rows.length) { res.json({ data: [], weekIso, generatedAt: null }); return; }
    ...
    res.json({ data, weekIso, generatedAt: rows[0]?.computedAt ?? null });
  } catch (e) { next(e); }
});
...
app.post('/api/v1/discover/:userId/superlike', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (existing) {
      ...
      grantPairMatchMilestones(prisma, userId, targetId);
...
app.post('/api/v1/safety/block', authMiddleware, idempotency(), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
