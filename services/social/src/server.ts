...
import { toPublicUser, withPublicAuthor } from '../../shared/src/security/publicUser';
...
app.get('/api/v1/discover', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    for (const u of users) {
      const userData = toPublicUser(u as any);
    ...
    // Emit only public-safe user fields — previously leaked email/phone/
    // googleId/isAdmin/etc. of every candidate to any authed swiper.
    const result = div.ranked.map(r => toPublicUser(r.user as any)) as any[];
...
app.get('/api/v1/matches', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    let result = matches.map(m => {
      ...
      const other = toPublicUser(otherUser as any);
      return {
        id: m.id,
        matchedUser: other,
        chatId: m.chat?.id,
...
app.get('/api/v1/matches/requests', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    res.json({ data: requests.map(r => ({ ...r, fromUser: toPublicUser(r.fromUser as any) })) });
  } catch (e) { next(e); }
});
...
app.get('/api/v1/matches/requests/sent', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    res.json({ data: requests.map(r => ({ ...r, toUser: toPublicUser(r.toUser as any) })) });
  } catch (e) { next(e); }
});
  ...
  try {
    ...
    const scored = recentVibes
      ...
      .map(v => {
        ...
        const userData = toPublicUser(v.user as any);
...
app.get('/api/v1/ai-match/suggestions', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    for (const c of candidates) {
      ...
      const userData = toPublicUser(c as any);
      topK.push(score, {
...
app.get('/api/v1/matches/incoming', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const data = filtered.map(like => {
      const userData = toPublicUser(like.fromUser as any);
    ...
    pendingRequests.forEach(req => {
      if (!data.find(d => d.user.id === req.fromUserId) && !matchedUserIds.has(req.fromUserId) && !blockedIds.has(req.fromUserId)) {
        const userData = toPublicUser(req.fromUser as any);
        data.push({
          id: req.id,
...
app.post('/api/v1/access/requests', authMiddleware, validate({ body: accessRequestCreateBodySchema }), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    // Accept both `toUserId` (canonical) and `targetUserId` (client naming).
    const body = req.body as { toUserId?: string; targetUserId?: string; field: string; message?: string };
    const toUserId = body.toUserId || body.targetUserId!;
    const { field, message } = body;
