...
const toastStyles: Record<ToastType, { bg: string; border: string; iconBg: string; iconColor: string; icon: ReactNode }> = {
 ...
 love: {
 bg: 'bg-white',
 border: 'border-rose-main/20',
 iconBg: 'bg-rose-vlight',
 iconColor: 'text-rose-main',
...
function ToastItem({ toast, onDismiss }: { toast: Toast; onDismiss: (id: string) => void }) {
 ...
 {toast.description && (
 <p className="text-xs text-text-muted mt-0.5 leading-relaxed">{toast.description}</p>
 )}
 {toast.action && (
 <button
 onClick={(e) => { e.stopPropagation(); toast.action!.onClick(); onDismiss(toast.id); }}
 className="mt-2 text-xs font-semibold text-rose-main hover:text-rose- transition-colors"
 >
 {toast.action.label}
 </button>
 )}
 </div>
 <button onClick={(e) => { e.stopPropagation(); onDismiss(toast.id); }} className="p-1 rounded-lg hover:bg-miamo-elevated transition-colors shrink-0">
