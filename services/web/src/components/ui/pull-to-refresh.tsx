...
export function PullToRefresh({ children, onRefresh, className }: PullToRefreshProps) {
 ...
 <motion.div
 style={{ opacity, scale }}
 className="absolute top-2 left-1/2 -translate-x-1/2 z-20 w-10 h-10 rounded-full bg-miamo-card shadow-lg border border-border flex items-center justify-center"
 >
 <motion.div style={{ rotate }}>
 <RefreshCw className={`w-5 h-5 text-rose ${refreshing ? 'animate-spin' : ''}`} />
 </motion.div>
 </motion.div>
