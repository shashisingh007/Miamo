...
import { Play, Heart, MessageCircle, Share2, Bookmark, Plus } from 'lucide-react';
    ...
    <Card hover className="overflow-hidden group">
      <div className="relative aspect-[9/16] max-h-[320px] bg-miamo-elevated">
        ...
        <div className="absolute bottom-0 inset-x-0 p-3">
          <p className="text-xs text-white line-clamp-2 mb-2">{video.title || video.description || ''}</p>
          <div className="flex items-center gap-2">
            <Avatar src={photo} name={author.displayName || 'User'} size="xs" verified={author.verified} />
            <span className="text-[11px] text-white/90 font-medium">{author.displayName || 'User'}</span>
          </div>
        </div>
      </div>
      <div className="px-3 py-2.5 space-y-2">
        <div className="flex items-center justify-between">
          <button onClick={toggleLike} className={cn('flex items-center gap-1 text-xs', liked ? 'text-lavender-400' : 'text-text-muted')}>
            <Heart className={cn('w-3.5 h-3.5', liked && 'fill-lavender-400')} /> {likeCount}
          </button>
          <button onClick={() => setShowComments(!showComments)} className={cn('flex items-center gap-1 text-xs', showComments ? 'text-lavender-400' : 'text-text-muted')}>
...
export default function VideosPage() {
  ...
  return (
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      <p className="text-xs text-lavender-400/70">💬 Comment on a video to express interest — if they respond, it&apos;s a match!</p>
      {loading ? (
