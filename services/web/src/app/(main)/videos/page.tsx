...
function VideoCard({ video }: { video: any }) {
 ...
 // click-matrix.md §5 rank 11: was `catch (e) {}` — like state advanced
 // client-side even on backend failure. Now optimistic w/ rollback + toast.
 const toggleLike = async () => {
  ...
  try {
   await api.reactToVideo(video.id);
  } catch (e: any) {
   setLiked(wasLiked);
   setLikeCount((c: number) => (wasLiked ? c + 1 : c - 1));
   logError('videos.reactToVideo', e);
 ...
 // click-matrix.md §5 rank 11: comments were silently swallowed on failure —
 // user typed something, saw the input clear, and had no idea it never posted.
 const submitComment = async () => {
  if (!commentText.trim() || posting) return;
  const text = commentText.trim();
  setPosting(true);
  try {
   await api.commentOnVideo(video.id, text);
   setCommentText('');
   setCommentCount((prev: number) => prev + 1);
   setShowComments(false);
  } catch (e: any) {
   logError('videos.commentOnVideo', e);
 ...
 <Button size="sm" onClick={submitComment} loading={posting} disabled={!commentText.trim()} className="h-8 text-xs px-2" aria-label="Post comment">Post</Button>
 </div>
 )}
 </div>
 </Card>
...
export default function VideosPage() {
 ...
 api.getVideos(params).then(res => setVideos(res.data || [])).catch((e) => logError('videos.load', e)).finally(() => setLoading(false));
 }, [filter]);
