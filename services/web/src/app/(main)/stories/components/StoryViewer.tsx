...
export function StoryViewer({ storyGroup, initialIndex = 0, onClose, onRefresh }: {
 ...
 // click-matrix.md §5 rank 15: menu-action loading + failure states
 const [menuBusy, setMenuBusy] = useState<null | 'delete' | 'postFeed' | 'copy' | 'save' | 'mute' | 'report'>(null);
 ...
 // click-matrix.md §5 rank 15 — user-visible failure toast + loading state on every menu action.
 ...
 console.error('[StoryViewer] deleteStory failed', e);
 ...
 // click-matrix.md §5 rank 15
 const handlePostToFeed = async () => {
 if (menuBusy) return;
 setMenuBusy('postFeed');
 try {
 await api.postStoryToFeed(story.id);
 toast.success('Posted to your feed');
 setShowMenu(false);
 } catch (e) {
 console.error('[StoryViewer] postStoryToFeed failed', e);
 ...
 console.error('[StoryViewer] copyLink failed', e);
 toast.error('Could not copy link', 'Clipboard access denied.');
 } finally {
 setMenuBusy(null);
 }
 };
 ...
 console.error('[StoryViewer] saveMedia failed', e);
 toast.error('Could not save media', 'Please try again.');
 } finally {
 setMenuBusy(null);
 }
 };
 ...
 setMenuBusy('mute');
 try {
 await (api as any).muteUser?.(author.id);
 toast.success(`Muted ${author.displayName?.split(' ')[0] || 'author'}`);
 setShowMenu(false);
 } catch (e) {
 console.error('[StoryViewer] muteUser failed', e);
 toast.error('Could not mute', 'Please try again.');
 } finally {
 setMenuBusy(null);
 }
 };

 // click-matrix.md §5 rank 15 — report action
 const handleReport = async () => {
 if (menuBusy) return;
 setMenuBusy('report');
 try {
 await api.reportUser({ reportedId: author.id, reason: 'inappropriate', targetType: 'story', targetId: story.id });
 toast.success('Report submitted', 'Our team will review this shortly.');
 setShowMenu(false);
 } catch (e) {
 console.error('[StoryViewer] reportUser failed', e);
 ...
 {isOwn && (
 <>
 <button onClick={handlePostToFeed} disabled={!!menuBusy} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-miamo-surface text-left text-sm disabled:opacity-60 disabled:cursor-not-allowed">
 <Share2 className="w-4 h-4 text-rose-main" /> {menuBusy === 'postFeed' ? 'Posting…' : 'Post to Feed'}
 </button>
 {story.mediaUrl && (
 <button onClick={handleSaveMedia} disabled={!!menuBusy} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-miamo-surface text-left text-sm disabled:opacity-60 disabled:cursor-not-allowed">
 <Download className="w-4 h-4 text-rose-main" /> {menuBusy === 'save' ? 'Saving…' : 'Save media'}
 </button>
 )}
 <button onClick={handleCopyLink} disabled={!!menuBusy} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-miamo-surface text-left text-sm disabled:opacity-60 disabled:cursor-not-allowed">
 <Link2 className="w-4 h-4 text-rose-main" /> {menuBusy === 'copy' ? 'Copying…' : 'Copy link'}
 </button>
 <button onClick={handleDelete} disabled={!!menuBusy} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-red-50 text-left text-sm text-red-500 disabled:opacity-60 disabled:cursor-not-allowed">
 <Trash2 className="w-4 h-4" /> {menuBusy === 'delete' ? 'Deleting…' : 'Delete Story'}
 </button>
 </>
 )}
 {!isOwn && (
 ...
 <button onClick={handleMuteAuthor} disabled={!!menuBusy} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-miamo-surface text-left text-sm disabled:opacity-60 disabled:cursor-not-allowed">
 <BellOff className="w-4 h-4 text-rose-main" /> {menuBusy === 'mute' ? 'Muting…' : `Mute ${author.displayName?.split(' ')[0] || 'author'}`}
 </button>
 <button onClick={handleReport} disabled={!!menuBusy} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-miamo-surface text-left text-sm disabled:opacity-60 disabled:cursor-not-allowed">
 <AlertCircle className="w-4 h-4 text-rose-main" /> {menuBusy === 'report' ? 'Reporting…' : 'Report'}
 </button>
 </>
 )}
