...
export default function DateIdeasPage() {
 ...
 <p className="text-text-primary/80 text-sm">{spotlight.desc}</p>
 </div>
 </div>
 <div className="flex gap-2 mt-3">
 <span className="px-2 py-0.5 rounded-md bg-miamo-card/20 text-xs font-bold">{spotlight.time}</span>
 <span className="px-2 py-0.5 rounded-md bg-miamo-card/20 text-xs font-bold">{spotlight.budget}</span>
 <span className="px-2 py-0.5 rounded-md bg-miamo-card/20 text-xs font-bold">{spotlight.difficulty}</span>
 </div>
 </div>
 <button onClick={() => setSpotlight(null)} className="absolute top-3 right-3 bg-miamo-card/20 rounded-lg p-1 hover:bg-miamo-card/30">
 <span className="text-text-primary text-xl leading-none">&times;</span>
 </button>
 </div>
 ...
 cat === c.id ? `border-transparent bg-gradient-to-r ${c.color} text-text-primary shadow-lg` : 'border-border text-text-secondary hover:bg-miamo-surface')}>
 ...
 <p className={cn('text-sm text-text-muted mt-1', !isExpanded && 'line-clamp-2')}>{idea.desc}</p>
 <div className="flex items-center gap-2 mt-3 flex-wrap">
 <span className={cn('px-2 py-0.5 rounded-md text-[10px] font-bold bg-gradient-to-r text-text-primary', catItem?.color)}>
 {catItem?.label}
 </span>
 ...
 <p className="text-text-muted font-semibold">No ideas in this category yet!</p>
 </motion.div>
 )}
 </div>
 </ErrorBoundary>
