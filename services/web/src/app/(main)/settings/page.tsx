...
import {
 ...
 MessageSquare, Heart, Zap, CreditCard, HelpCircle, Sliders, Languages,
 ...
 return (
  <Portal>
   ...
   <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
    className="fixed inset-x-4 top-[20%] max-w-sm mx-auto bg-miamo-card dark:bg-[#181A1F] border border-border dark:border-[#2A2D34] rounded-2xl shadow-2xl z-50 p-6 text-center">
    ...
    <h3 className="text-base font-bold text-text-primary dark:text-[#F4F1EC] mb-2">Two-Factor Authentication</h3>
    <p className="text-sm text-text-muted dark:text-[#B8B3AC] mb-2">Protect your account with an extra layer of security.</p>
    <div className="bg-miamo-elevated dark:bg-[#1F2229] rounded-xl p-4 mb-4 border border-border/30 dark:border-[#2A2D34]/60">
     <div className="w-32 h-32 bg-white dark:bg-[#0E0F12] rounded-xl mx-auto mb-2 flex items-center justify-center border border-border/50 dark:border-[#2A2D34]">
      <KeyRound className="w-8 h-8 text-text-muted/30" />
     </div>
     <p className="text-xs text-text-muted">Scan with your authenticator app</p>
    </div>
    <div className="bg-rose-soft/40 dark:bg-rose-main/10 border border-rose-main/15 rounded-xl p-3 mb-4">
     <p className="text-xs text-rose-main font-medium">Coming in the next update</p>
    </div>
...
export default function SettingsPage() {
 ...
 const toggleServer = async (key: keyof typeof server) => {
  ...
  } catch { setServer((s) => ({ ...s, [key]: !newVal })); }
 };
 ...
 const handleUnblock = async (userId: string) => {
  try { await api.unblockUser(userId); setBlockList((p) => p.filter((b) => b.id !== userId)); showToast('Unblocked', 'success'); }
  catch {}
 };
 const loadBlockList = () => api.getBlockList().then((r) => setBlockList(r.data || [])).catch((e) => logError('settings.getBlockList', e));
       ...
       <Card className="p-6 md:p-7 bg-miamo-card dark:bg-[#181A1F] border border-border dark:border-[#2A2D34]">
        ...
        {activeSection === 'account' && (
         <div className="space-y-1">
          ...
          <SettingRow icon={Shield} label="Two-factor authentication" description="Add extra security">
           <Button variant="ghost" size="sm" onClick={() => setTwoFactorModal(true)}>Enable</Button>
          </SettingRow>
          ...
          <SettingRow icon={Sparkles} label="Connected accounts" description="Google, Apple, Facebook — link soon">
        ...
        {activeSection === 'appearance' && (
         <div className="space-y-1">
          ...
          <SettingRow label="Accent color" description="Coming soon">
           <div className="flex gap-1.5">
            {['#C97856', '#B58A4E', '#8B5E83', '#3F6E6A', '#5B7CB8'].map((c, i) => (
             <div key={c} className={cn('w-5 h-5 rounded-full border border-border/40', i === 0 && 'ring-2 ring-offset-2 ring-offset-miamo-card dark:ring-offset-[#181A1F] ring-rose-main')} style={{ background: c }} />
            ))}
           </div>
          </SettingRow>
         </div>
        ...
        {activeSection === 'safety' && (
         <div className="space-y-1">
          ...
          <SettingRow icon={Shield} label="Block list" description={`${blockList.length} blocked`}>
           <Button variant="ghost" size="sm" onClick={loadBlockList}>Manage</Button>
          </SettingRow>
          {blockList.length > 0 && (
           <div className="space-y-2 py-2">
            {blockList.map((b: any) => (
             <div key={b.id} className="flex items-center justify-between bg-miamo-elevated/50 dark:bg-[#1F2229]/60 px-3 py-2 rounded-lg">
              <span className="text-xs text-text-secondary dark:text-[#B8B3AC]">{b.displayName || b.username || b.id}</span>
        ...
        {activeSection === 'subscription' && (
         <div className="space-y-4">
          <div className="rounded-2xl p-5 bg-gradient-to-br from-rose-main to-rose-light text-white shadow-rose">
           ...
           <h3 className="text-2xl font-semibold">Miamo Premium</h3>
           <p className="text-xs opacity-90 mt-1">Renews on the 15th · $14.99 / month</p>
          </div>
          <div className="space-y-1">
           <SettingRow icon={CreditCard} label="Manage billing" description="Payment method, history, invoices">
            <Button variant="ghost" size="sm" onClick={() => router.push('/premium')}>Open</Button>
           </SettingRow>
           <SettingRow icon={Sparkles} label="Restore purchases" description="Recover entitlements from another device">
            <Button variant="ghost" size="sm" onClick={() => showToast('Restored', 'success')}>Restore</Button>
           </SettingRow>
           <SettingRow label="Cancel subscription" description="Keep premium until period ends">
        ...
        {activeSection === 'help' && (
         <div className="space-y-1">
          ...
          <SettingRow icon={Mail} label="Contact support" description="We reply within 24h"><Button variant="ghost" size="sm">Email us</Button></SettingRow>
          <SettingRow icon={MessageSquare} label="Send feedback" description="Tell us what could be better"><Button variant="ghost" size="sm">Share</Button></SettingRow>
          <SettingRow icon={Sparkles} label="What’s new" description="See the latest updates"><Button variant="ghost" size="sm">View</Button></SettingRow>
          <SettingRow icon={Heart} label="Rate Miamo" description="Help us spread the word"><Button variant="ghost" size="sm">Rate</Button></SettingRow>
          ...
          <SettingRow icon={Trash2} label="Delete account" description="Permanently delete everything. This is forever.">
           {showDeleteConfirm ? (
            <div className="flex items-center gap-2">
             <span className="text-xs text-rose-main">Are you sure?</span>
