...
import { useTrackPageView, useTrackScrollDepth, trackSettingsChange, trackClick } from '@/hooks/useTrackActivity';
  ...
  return (
    <>
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div className="w-14 h-14 rounded-2xl bg-lavender-400/10 flex items-center justify-center mx-auto mb-4">
          <KeyRound className="w-7 h-7 text-lavender-400" />
        </div>
    ...
    <div className="max-w-4xl mx-auto p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[220px_1fr] gap-6">
        <nav className="flex lg:flex-col gap-1 overflow-x-auto lg:overflow-visible no-scrollbar">
          {sections.map(s => (
            <button key={s.id} onClick={() => setActiveSection(s.id)} className={cn('flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm font-medium whitespace-nowrap transition-all', activeSection === s.id ? 'bg-lavender-400/10 text-lavender-400' : 'text-text-muted hover:text-text-secondary hover:bg-miamo-elevated/50')}>
              ...
              <SettingRow label="Profile visibility" description="Who can see your profile">
                <div className="flex gap-1.5">
                  {(['public','matches','private'] as const).map(v => (
                    <button key={v} onClick={() => updatePref('profileVisibility',v)} className={cn('px-3 py-1.5 rounded-lg text-[11px] font-medium border transition-all', settings.profileVisibility===v ? 'bg-lavender-400/10 border-lavender-400/30 text-lavender-400' : 'border-border text-text-muted')}>{v.charAt(0).toUpperCase()+v.slice(1)}</button>
                  ))}
                </div>
              </SettingRow>
              ...
              <SettingRow label="Theme" description={`Current: ${theme.charAt(0).toUpperCase() + theme.slice(1)}`}>
                <div className="flex gap-2">
                  {[{ icon: Moon, label: 'dark' as const }, { icon: Sun, label: 'light' as const }, { icon: Monitor, label: 'system' as const }].map(t => (
                    <button key={t.label} onClick={() => setTheme(t.label)} className={cn('p-2 rounded-lg border transition-all', theme === t.label ? 'bg-lavender-400/10 border-lavender-400/30 text-lavender-400' : 'border-border text-text-muted')}>
              ...
              <SettingRow label="Font size" description={`Current: ${settings.fontSize}`}>
                <div className="flex gap-1.5">
                  {[{v:'small',l:'S'},{v:'medium',l:'M'},{v:'large',l:'L'}].map(f => (
                    <button key={f.v} onClick={() => updatePref('fontSize',f.v)} className={cn('w-8 h-8 rounded-lg border flex items-center justify-center text-xs font-bold transition-all', settings.fontSize===f.v ? 'bg-lavender-400/10 border-lavender-400/30 text-lavender-400' : 'border-border text-text-muted')}>{f.l}</button>
                  ))}
                </div>
              </SettingRow>
              ...
              <SettingRow label="Chat bubble style">
                <div className="flex gap-1.5">
                  {(['modern','classic','minimal'] as const).map(s => (
                    <button key={s} onClick={() => updatePref('chatBubbleStyle',s)} className={cn('px-3 py-1.5 rounded-lg text-[11px] font-medium border transition-all', settings.chatBubbleStyle===s ? 'bg-lavender-400/10 border-lavender-400/30 text-lavender-400' : 'border-border text-text-muted')}>{s.charAt(0).toUpperCase()+s.slice(1)}</button>
                  ))}
                </div>
              </SettingRow>
              <SettingRow label="Accent color">
                <div className="flex gap-1.5">
                  {['bg-lavender-400','bg-violet-deep','bg-sky-400','bg-emerald-400','bg-amber-400'].map(c => (
                    <div key={c} className={cn('w-5 h-5 rounded-full', c, c === 'bg-lavender-400' && 'ring-2 ring-offset-2 ring-offset-miamo-card ring-lavender-400')} />
                  ))}
                </div>
              </SettingRow>
            </div>
              ...
              <SettingRow label="Date format">
                <div className="flex gap-1.5">
                  {[{v:'relative',l:'2h ago'},{v:'absolute',l:'May 17'},{v:'short',l:'5/17'}].map(d => (
                    <button key={d.v} onClick={() => updatePref('dateFormat',d.v)} className={cn('px-3 py-1.5 rounded-lg text-[11px] font-medium border transition-all', settings.dateFormat===d.v ? 'bg-lavender-400/10 border-lavender-400/30 text-lavender-400' : 'border-border text-text-muted')}>{d.l}</button>
                  ))}
                </div>
              </SettingRow>
            </div>
