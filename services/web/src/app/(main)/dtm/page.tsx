...
import { FamilyBriefSharesPanel } from './components/FamilyBriefSharesPanel';
...
export default function DtmPage() {
  ...
  // click-matrix.md §5 rank 22: loading state prevents accidental double-tap
  // while the (pending) endpoint round-trips. Answer is preserved on failure.
  ...
  // click-matrix.md §5 rank 22: loading + preserve-answer-on-failure. The
  // real backend POST is still TODO(v6.7); until then the "await" is a
  // one-tick guard against accidental double-taps, and the answer is only
  // cleared *after* the local update path succeeds.
  const handleAnswer = async () => {
    if (!current || !answer.trim() || submittingAnswer) return;
    const draft = answer;
    setSubmittingAnswer(true);
    try {
      if (initialAnswerRef.current && initialAnswerRef.current !== draft) {
      ...
      initialAnswerRef.current = draft;
      ...
      // Once wired, wrap the network call in try/catch so we can restore the
      // draft answer and toast on failure without advancing the question.
      setBatchActed((n) => n + 1);
      ...
      console.error('[dtm] handleAnswer failed', e);
      toast.error('Could not save answer', 'Your answer was kept — try again.');
      setAnswer(draft);
    } finally {
