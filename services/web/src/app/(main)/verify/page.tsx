      ...
      <div className="rounded-2xl border border-neutral-200 dark:border-neutral-700 p-6 bg-white dark:bg-neutral-900">
        {step === 'email-send' && (
          <Section title="Step 1: Verify your email" desc="We'll send a 6-digit code to your registered email.">
            <button onClick={sendEmail} disabled={busy} className="px-4 py-2 rounded-lg bg-rose-500 text-white disabled:opacity-50">Send code</button>
          </Section>
        )}
        {step === 'email-verify' && (
          <Section title="Enter the email code" desc={emailDevCode ? `Dev code: ${emailDevCode}` : 'Check your inbox'}>
            ...
            <button onClick={() => verifyEmail(emailCode)} disabled={busy || emailCode.length !== 6} className="mt-4 w-full px-4 py-2 rounded-lg bg-rose-500 text-white disabled:opacity-50">Verify</button>
            <button onClick={sendEmail} disabled={busy} className="mt-2 text-xs text-neutral-500 underline">Resend code</button>
          </Section>
        )}
        {step === 'phone-input' && (
          <Section title="Step 2: Verify your phone" desc="We'll text you a 6-digit code.">
            <PhoneInput value={phone} onChange={setPhone} />
            <button onClick={sendPhone} disabled={busy} className="mt-4 w-full px-4 py-2 rounded-lg bg-rose-500 text-white disabled:opacity-50">Send code</button>
          </Section>
        )}
        {step === 'phone-verify' && (
          <Section title="Enter the SMS code" desc={phoneDevCode ? `Dev code: ${phoneDevCode}` : 'Check your messages'}>
            ...
            <button onClick={() => verifyPhone(phoneCode)} disabled={busy || phoneCode.length !== 6} className="mt-4 w-full px-4 py-2 rounded-lg bg-rose-500 text-white disabled:opacity-50">Verify</button>
            <button onClick={() => setStep('phone-input')} className="mt-2 text-xs text-neutral-500 underline">Change phone</button>
          </Section>
        )}
        {step === 'selfie' && (
