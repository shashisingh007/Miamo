...
export default function ProfilePage() {
 ...
 // click-matrix.md §5 rank 19: was `.catch(() => {})` — if profile load failed,
 // the skeleton spun forever and the user had no idea. Log + toast + stop.
 const loadProfile = () => {
  setLoading(true);
  api
   .getMyProfile()
   ...
   .catch((e: any) => {
    logError('profile.loadProfile', e);
    toast.error('Could not load profile', 'Pull to refresh or try again.');
   })
 ...
 {/* click-matrix.md §5 rank 2: wire dead "Start Verification" button to /verify */}
 <Button
  size="sm"
  className="w-full"
 ...
 {/* Phase F — Trust Score breakdown (flag-gated; renders nothing when
     FEATURE_TRUST_SCORE_ENABLED=0). Placed BELOW the Verification CTA so
     the "Start Verification" push stays the top visual for unverified
     users; the score card gives verified/near-verified users an
     always-fresh view of their trust standing. */}
