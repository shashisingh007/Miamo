...
export default function OnboardingPage() {
  ...
  async function patchProfile(patch: Record<string, any>): Promise<boolean> {
    setSaving('profile');
    try { await api.updateProfile(patch); await load(); return true; }
    catch (e: any) {
  ...
  async function patchMp(patch: Record<string, any>): Promise<boolean> {
    setSaving('mp');
    try { await api.updateMatrimonialProfile({ ...(mp ?? {}), ...patch }); await load(); return true; }
    catch (e: any) {
  ...
  async function saveInterests(next: string[]): Promise<boolean> {
    setSaving('interests');
    try { await api.updateInterests(next); await load(); return true; }
    catch (e: any) {
