...
export default function MatchesPage() {
 ...
 // click-matrix.md §5 rank 7: was showing "Resumed N" success toast even if
 // 4 of 5 failed. Track failures explicitly and warn the user when partial.
 const handleBulkResume = async () => {
  ...
  for (const id of ids) {
   try {
    ...
    failed++;
    logError('matches.bulkResume', e);
   }
  }
  if (failed === 0) {
   showToast(`Resumed ${resumed} profile${resumed !== 1 ? 's' : ''}`, 'success');
  } else if (resumed === 0) {
   showToast(`Could not resume ${failed} profile${failed !== 1 ? 's' : ''}`, 'error');
  } else {
   showToast(`Resumed ${resumed}, ${failed} failed — try again`, 'error');
  }
  clearSelection();
  loadData();
 };
 ...
 // click-matrix.md §5 rank 4: removed handleVideoCall — video calls are not
 // a v1 feature and the button was a "coming soon" lie. The Video button on
 // MatchCard is now gated off (see MatchCard.tsx onVideoCall optional prop).
 ...
 {pinnedMatches.map(match => <MatchCard key={match.id} match={match} onOpenMenu={openMenu} onChat={handleChat} />)}
 ...
 {unpinnedMatches.map(match => <MatchCard key={match.id} match={match} onOpenMenu={openMenu} onChat={handleChat} />)}
 </div>
 )}
 </div>
 )}
