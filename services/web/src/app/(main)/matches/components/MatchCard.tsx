...
export function TooltipButton({
 ...
 active ? 'bg-miamo-surface text-text-primary' : 'bg-transparent text-text-muted hover:text-text-secondary hover:bg-miamo-surface',
 className,
 )}>
 ...
 className="absolute -top-9 left-1/2 -translate-x-1/2 whitespace-nowrap px-2.5 py-1 rounded-lg bg-miamo-card text-text-primary text-[10px] font-bold shadow-lg z-50 pointer-events-none"
 >
 {label}
 <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-2 h-2 bg-miamo-card rotate-45" />
 </motion.div>
 )}
 </AnimatePresence>
 </div>
...
export function HeldItemMenu({ userId, onResume, onReport, onBlock, onUnmatch }: { userId: string; onResume: () => void; onReport: () => void; onBlock: () => void; onUnmatch: () => void }) {
 ...
 <button onClick={(e) => { e.stopPropagation(); setOpen(!open); }} className="p-2 rounded-lg hover:bg-miamo-surface text-text-muted hover:text-text-secondary transition">
...
export function IncomingCard({ item, onClick }: { item: any; onClick: () => void }) {
 ...
 whileTap={{ scale: 0.99 }}
 onClick={onClick}
 className="w-full text-left rounded-2xl border border-border bg-miamo-surface/50 hover:bg-miamo-surface hover:border-border transition-all overflow-hidden"
 >
 <div className="flex items-center gap-4 p-4">
 {/* Avatar */}
 <div className="relative flex-shrink-0">
 <div className="w-16 h-16 rounded-2xl overflow-hidden bg-miamo-surface">
 {photo ? (
 <img loading="lazy" src={photo} alt={name} className="w-full h-full object-cover" />
 ) : (
 <div className="w-full h-full flex items-center justify-center text-xl font-black text-text-secondary">{name[0]}</div>
 )}
 </div>
 ...
 <span className="text-[10px] text-text-secondary font-medium">{timeAgo()}</span>
 <div className="w-7 h-7 rounded-lg bg-miamo-surface0/10 flex items-center justify-center">
 <Eye className="w-3.5 h-3.5 text-rose-light/60" />
 </div>
 </div>
 </div>
 </motion.button>
...
export function MatchCard({ match, onOpenMenu, onChat, onVideoCall }: { match: any; onOpenMenu: (id: string, e: React.MouseEvent) => void; onChat: (m: any) => void; onVideoCall?: () => void }) {
 ...
 className={cn('group rounded-2xl border transition-all cursor-pointer', isPinned ? 'bg-miamo-surface border-border' : 'bg-miamo-surface/50 border-border hover:bg-miamo-surface hover:border-border')}
...
export function ContextMenu({
 ...
 className="w-full flex items-center gap-3 px-4 py-2.5 text-left hover:bg-miamo-surface transition-colors">
