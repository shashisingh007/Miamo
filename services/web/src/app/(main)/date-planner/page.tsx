...
function StepIndicator({ current, total }: { current: number; total: number }) {
 ...
 i === current ? 'w-8 bg-gradient-rose shadow-lg shadow-medium' :
 i < current ? 'w-2 bg-rose-light' : 'w-2 bg-border'
 )}
 />
 ))}
 </div>
...
export default function DatePlannerPage() {
 ...
 className="w-14 h-14 rounded-2xl bg-gradient-to-br from-rose to-rose- flex items-center justify-center shadow-xl shadow-medium/50"
 >
 <CalendarHeart className="w-7 h-7 text-text-primary" />
 </motion.div>
 <div>
 <h1 className="text-2xl font-black text-text-primary tracking-tight">Date Planner</h1>
 <p className="text-sm text-text-muted">Plan the perfect date experience ✨</p>
 </div>
 <div className="ml-auto">
 <Button onClick={() => { setCreating(true); resetForm(); }} className="gap-2 shadow-lg shadow-medium/40">
 ...
 className="bg-miamo-card rounded-3xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-hidden flex flex-col"
 onClick={e => e.stopPropagation()}
 >
 {/* Modal header */}
 <div className="p-6 pb-0">
 <div className="flex items-center justify-between mb-4">
 <h2 className="text-xl font-black text-text-primary">Plan Your Date</h2>
 <button onClick={() => setCreating(false)} className="w-8 h-8 rounded-full bg-miamo-surface flex items-center justify-center hover:bg-miamo-elevated transition-all">
 ...
 <h3 className="text-lg font-bold text-text-secondary mb-1">Who&apos;s the lucky one? 💕</h3>
 <p className="text-sm text-text-muted mb-4">Choose a match to plan a date with</p>
 ...
 selected ? 'border-rose-main bg-miamo-surface shadow-lg shadow-soft' : 'border-transparent hover:bg-miamo-surface')}
 >
 <Avatar src={photo} name={user.displayName || 'Match'} size="md" />
 <div className="flex-1 min-w-0">
 <p className="font-semibold text-text-primary truncate">{user.displayName || 'Match'}</p>
 <p className="text-xs text-text-muted">@{user.username || 'user'}</p>
 </div>
 {selected && (
 <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="w-8 h-8 rounded-full bg-miamo-surface flex items-center justify-center">
 ...
 <Heart className="w-10 h-10 text-text-primary mx-auto mb-2" />
 <p className="text-sm text-text-muted">No matches yet. Get swiping!</p>
 </div>
 )}
 </div>
 </motion.div>
 ...
 <h3 className="text-lg font-bold text-text-secondary mb-1">Set the vibe 🎭</h3>
 <p className="text-sm text-text-muted mb-4">What kind of energy are you going for?</p>
 ...
 selectedVibe === vibe.id ? 'border-rose-main shadow-lg' : 'border-transparent')}
 >
 ...
 <p className="font-bold text-sm text-text-secondary">{vibe.label}</p>
 </motion.button>
 ))}
 </div>
 </motion.div>
 ...
 <h3 className="text-lg font-bold text-text-secondary mb-1">Pick a spot 📍</h3>
 <p className="text-sm text-text-muted mb-4">Where do you want to go?</p>
 <div className="grid grid-cols-2 gap-3">
 {VENUES.map(v => {
 const Icon = v.icon;
 ...
 sel ? 'border-rose-main bg-miamo-surface shadow-lg' : 'border-border bg-miamo-card hover:bg-miamo-surface')}
 >
 <motion.div animate={sel ? { rotateY: 360 } : {}} transition={{ duration: 0.6 }}
 className={cn('w-12 h-12 rounded-xl flex items-center justify-center', sel ? 'bg-miamo-surface' : 'bg-miamo-surface')}>
 <Icon className={cn('w-6 h-6', v.color)} />
 </motion.div>
 <p className="font-semibold text-sm text-text-secondary">{v.label}</p>
 </motion.button>
 );
 })}
 </div>
 ...
 <h3 className="text-lg font-bold text-text-secondary mb-1">When & How? ⏰</h3>
 <p className="text-sm text-text-muted mb-4">Pick the perfect time</p>
 ...
 sel ? 'border-rose-main bg-miamo-surface' : 'border-border hover:bg-miamo-surface')}>
 ...
 <div>
 <p className="text-sm font-bold text-text-secondary mb-2">Budget 💰</p>
 <div className="flex flex-wrap gap-2">
 {BUDGETS.map(b => (
 ...
 selectedBudget === b ? 'border-rose-main bg-miamo-surface text-rose-dark' : 'border-border text-text-secondary hover:bg-miamo-surface')}>
 {b}
 </motion.button>
 ))}
 </div>
 ...
 <h3 className="text-lg font-bold text-text-secondary mb-1">Fun Extras 🎉</h3>
 <p className="text-sm text-text-muted mb-4">Add activities to make it special</p>
 ...
 selectedActivities.includes(a) ? 'border-rose-main bg-miamo-surface text-rose-dark' : 'border-border text-text-muted hover:bg-miamo-surface')}>
 {a}
 </motion.button>
 ))}
 </div>
 ...
 className="w-full p-3 rounded-xl border border-border bg-miamo-surface text-sm resize-none h-24 focus:outline-none focus:ring-2 focus:ring-rose/20"
 />
 </motion.div>
 )}
 </AnimatePresence>
 ...
 <Button onClick={handleCreate} className="flex-1 gap-2 bg-gradient-rose shadow-lg">
 ...
 <motion.h2 initial={{ y: 20 }} animate={{ y: 0 }} className="text-3xl font-black text-text-primary mb-2">
 Date Planned!
 </motion.h2>
 <p className="text-text-primary/70 text-lg">Get ready for something magical ✨</p>
 </motion.div>
 </motion.div>
 )}
 </AnimatePresence>
 ...
 <h3 className="font-bold text-text-primary">{vibe?.label || 'Date'} with {plan.matchName}</h3>
 ...
 <span key={a} className="px-2 py-1 rounded-lg bg-miamo-surface text-[10px] font-semibold text-rose">{a}</span>
 ))}
 </div>
 )}
 {plan.notes && <p className="text-xs text-text-muted mt-2 italic">&ldquo;{plan.notes}&rdquo;</p>}
 </div>
 ...
 <CalendarHeart className="w-12 h-12 text-rose-light" />
 </motion.div>}
