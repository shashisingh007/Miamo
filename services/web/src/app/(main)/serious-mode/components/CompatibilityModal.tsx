  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }}
        className="bg-white dark:bg-gray-900 rounded-3xl max-w-lg w-full max-h-[90vh] overflow-y-auto border border-zinc-200 dark:border-zinc-700 shadow-2xl"
        onClick={(e: any) => e.stopPropagation()}>
        <div className="p-6 space-y-5">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-bold text-zinc-900 dark:text-white flex items-center gap-2"><BarChart3 className="w-5 h-5 text-purple-500" /> Compatibility Report</h2>
            <button onClick={onClose} className="w-8 h-8 rounded-full bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center hover:bg-zinc-200 dark:hover:bg-zinc-700"><X className="w-4 h-4 text-zinc-600 dark:text-zinc-400" /></button>
          </div>
          <p className="text-sm text-zinc-500 dark:text-zinc-400">Match with <span className="font-semibold text-zinc-800 dark:text-zinc-200">{partnerName}</span></p>

          {/* Composite Score */}
          <div className="text-center py-6 bg-zinc-50 dark:bg-zinc-800 rounded-2xl border border-zinc-100 dark:border-zinc-700">
            <div className={`text-5xl font-black ${scoreColor}`}>{compositeScore}%</div>
            <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-2">Overall Compatibility</p>
            <div className="w-48 h-2 bg-zinc-200 dark:bg-zinc-700 rounded-full mx-auto mt-3 overflow-hidden">
          ...
          {kundli && (
            <div className="space-y-3">
              <h3 className="text-sm font-bold text-zinc-900 dark:text-white flex items-center gap-2">🕉 Ashtakoota — {kundli.totalPoints}/{kundli.maxPoints} Points</h3>
              ...
              <div className="space-y-2">
                {kundli.koots?.map((k: any) => (
                  <div key={k.name} className="flex items-center gap-3">
                    <span className="text-xs font-semibold text-zinc-700 dark:text-zinc-300 w-24">{k.name}</span>
                    ...
                    <span className="text-xs font-bold text-zinc-600 dark:text-zinc-400 w-10 text-right">{k.score}/{k.max}</span>
                  </div>
                ))}
              </div>
              ...
              {kundli.insights && kundli.insights.length > 0 && (
                <div className="mt-3 space-y-2">
                  <h4 className="text-xs font-bold text-zinc-700 dark:text-zinc-300 flex items-center gap-1.5">💡 Insights</h4>
          ...
          {numerology && (
            <div className="space-y-3 pt-3 border-t border-zinc-200 dark:border-zinc-700">
              <h3 className="text-sm font-bold text-zinc-900 dark:text-white flex items-center gap-2"><Hash className="w-4 h-4 text-purple-500" /> Numerology Match — {numerology.score}%</h3>
