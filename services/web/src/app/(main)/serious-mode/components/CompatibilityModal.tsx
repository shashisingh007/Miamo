...
export function CompatibilityModal({ data, onClose }: { data: any; onClose: () => void }) {
 ...
 const scoreColor = compositeScore >= 75 ? 'text-rose-main' : compositeScore >= 55 ? 'text-rose-main' : compositeScore >= 40 ? 'text-rose-main' : 'text-red-500';
 const scoreBg = compositeScore >= 75 ? 'from-rose-main' : compositeScore >= 55 ? 'from-rose-main' : compositeScore >= 40 ? 'from-rose-main' : 'from-red-500';
 ...
 <h2 className="text-lg font-bold text-zinc-900 flex items-center gap-2"><BarChart3 className="w-5 h-5 text-rose-main" /> Compatibility Report</h2>
 ...
 kundli.level === 'excellent' ? 'bg-rose-soft text-rose-dark' : kundli.level === 'good' ? 'bg-rose-soft text-rose-dark' : kundli.level === 'average' ? 'bg-rose-soft text-rose-dark' : 'bg-red-50 text-red-700'
 )}>{kundli.verdict}</p>
 ...
 <div className="h-full bg-gradient-to-r from-rose-alt to-rose-main rounded-full" style={{ width: `${(k.score / k.max) * 100}%` }} />
 </div>
 ...
 isWarning ? 'bg-rose-soft border-rose-light' : isPositive ? 'bg-rose-soft border-rose-light' : 'bg-rose-soft border-rose-soft'
 )}>
 <span className={cn('text-xs mt-0.5', isWarning ? 'text-rose-main' : isPositive ? 'text-rose-main' : 'text-rose-main')}>{isWarning ? '⚠️' : isPositive ? '✨' : '•'}</span>
 <p className={cn('text-xs leading-relaxed', isWarning ? 'text-rose-dark' : isPositive ? 'text-rose-dark' : 'text-rose-dark')}>{isWarning || isPositive ? insight.slice(2).trim() : insight}</p>
 </div>
 );
 })}
 </div>
 ...
 <div className="bg-rose-soft border border-rose-light rounded-xl p-3 flex items-start gap-2">
 <AlertTriangle className="w-4 h-4 text-rose-main mt-0.5 shrink-0" />
 <p className="text-xs text-rose-dark"><span className="font-bold">Manglik Dosha:</span> One partner is Manglik. Consider remedies.</p>
 </div>
 )}
 ...
 <h3 className="text-sm font-bold text-zinc-900 flex items-center gap-2"><Hash className="w-4 h-4 text-rose-main" /> Numerology Match — {numerology.score}%</h3>
 <div className="grid grid-cols-2 gap-3">
 <div className="bg-rose-soft rounded-xl p-3 text-center border border-rose-soft">
 <p className="text-2xl font-black text-rose-main">{numerology.myNumber}</p>
 <p className="text-[10px] text-rose-main font-semibold mt-1">Your Life Path</p>
 </div>
 <div className="bg-rose-soft rounded-xl p-3 text-center border border-rose-soft">
 <p className="text-2xl font-black text-rose-main">{numerology.partnerNumber}</p>
 <p className="text-[10px] text-rose-main font-semibold mt-1">Partner Life Path</p>
 </div>
 </div>
 </div>
 )}
