...
export function ProfileEditor({
 ...
 <h2 className="text-lg font-bold text-zinc-900 flex items-center gap-2"><FileText className="w-5 h-5 text-rose-main" /> My Bio Data</h2>
 <div className="flex gap-2">
 <button onClick={saveProfile} disabled={saving}
 className="px-5 py-2.5 rounded-xl text-xs font-bold bg-gradient-to-r from-rose-main to-rose-main text-text-primary hover:shadow-lg transition disabled:opacity-50">
 {saving ? 'Saving...' : 'Save Profile'}
 </button>
 {myProfile.fullName && (
 <button onClick={() => { setPreviewTemplate(myProfile.bioDataTemplate || 'royal-rajasthani'); setShowPreview(true); }}
 className="px-4 py-2.5 rounded-xl text-xs font-semibold text-rose-dark bg-rose-soft border border-rose-light hover:bg-rose-soft transition flex items-center gap-1.5">
 ...
 i === bioDataStep ? 'bg-rose-soft text-rose-dark border-rose-light' : i < bioDataStep ? 'bg-rose-soft text-rose-dark border-rose-light' : 'text-zinc-500 border-zinc-200 hover:border-zinc-300')}>
 ...
 <input type="checkbox" checked={myProfile.horoscopeMatch || false} onChange={(e: any) => updateField('horoscopeMatch', e.target.checked)} className="w-4 h-4 rounded bg-zinc-200 text-rose-main focus:ring-rose-main/40" />
 ...
 <input type="checkbox" checked={myProfile[item.key] || false} onChange={(e: any) => updateField(item.key, e.target.checked)} className="w-4 h-4 rounded bg-zinc-200 text-rose-main focus:ring-rose-main/40" />
 ...
 <div className="bg-gradient-to-br from-rose-soft to-rose-soft rounded-2xl border border-rose-light p-5 space-y-4 shadow-sm">
 <div className="flex items-center justify-between">
 <h3 className="text-sm font-bold text-zinc-900 flex items-center gap-2">🎨 Choose Your BioData Template</h3>
 <span className="text-[10px] px-2 py-0.5 bg-rose-soft text-rose-dark rounded-full font-medium">Profile {profileCompletion}% complete</span>
 </div>
 ...
 myProfile?.bioDataTemplate === t.id ? 'border-rose-main bg-miamo-card shadow-md ring-2 ring-rose-light' : 'border-zinc-200 bg-miamo-card hover:border-rose-light')}>
 ...
 {t.premium && <span className="text-[9px] px-1 py-0.5 bg-rose-soft text-rose-dark rounded font-bold">⭐</span>}
 </div>
 ...
 {myProfile?.bioDataTemplate === t.id && <div className="absolute top-1.5 right-1.5 w-4 h-4 bg-rose-main rounded-full flex items-center justify-center"><span className="text-text-primary text-[9px]">✓</span></div>}
 </button>
 ))}
 </div>
 {myProfile?.bioDataTemplate && (
 <button onClick={() => { setPreviewTemplate(myProfile.bioDataTemplate); setShowPreview(true); }}
 className="w-full py-2.5 rounded-xl text-xs font-semibold text-rose-dark bg-miamo-card border border-rose-light hover:bg-rose-soft transition flex items-center justify-center gap-2">
 ...
 <div className="flex gap-2">
 <button onClick={saveProfile} disabled={saving} className="px-5 py-2.5 rounded-xl text-xs font-bold bg-gradient-to-r from-rose-main to-rose-main text-text-primary hover:shadow-lg transition disabled:opacity-50">
 {saving ? 'Saving...' : 'Save'}
 </button>
 {bioDataStep < 5 && (
