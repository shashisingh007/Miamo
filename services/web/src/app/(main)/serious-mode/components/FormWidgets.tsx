...
export function Input({ value, onChange, placeholder, type = 'text', disabled }: { value: string; onChange: (v: string) => void; placeholder?: string; type?: string; disabled?: boolean }) {
 ...
 className="w-full bg-zinc-50 border border-zinc-200 rounded-xl px-3.5 py-2.5 text-sm text-zinc-900 placeholder:text-zinc-400 focus:ring-2 focus:ring-rose-main/40 focus:border-rose-alt outline-none transition disabled:opacity-50" />
 );
}
...
export function Select({ value, onChange, options, placeholder }: { value: string; onChange: (v: string) => void; options: string[]; placeholder?: string }) {
 ...
 <option value="" className="bg-miamo-card text-zinc-400">{placeholder || 'Select...'}</option>
 {options.map(o => <option key={o} value={o} className="bg-miamo-card text-zinc-900">{o}</option>)}
 </select>
 );
}
...
export function Textarea({ value, onChange, placeholder, rows = 3 }: { value: string; onChange: (v: string) => void; placeholder?: string; rows?: number }) {
 ...
 className="w-full bg-zinc-50 border border-zinc-200 rounded-xl px-3.5 py-2.5 text-sm text-zinc-900 placeholder:text-zinc-400 focus:ring-2 focus:ring-rose-main/40 focus:border-rose-alt outline-none transition resize-none" />
 );
}
