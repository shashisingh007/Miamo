...
// v1.2 session 14 — Task 1: wire MatchSuccessModal into discover so the
// server-returned `isFirstMatch` + `isMutual` from `POST /discover/like`
// actually opens the celebration surface (session 13 shipped the API +
// modal component; the page-consumer was deferred).
import { MatchSuccessModal, type MatchedUser } from './components/MatchSuccessModal';
...
export default function DiscoverPage() {
 ...
 // v3.6.0 — Tab switcher between the live Discover deck and the Weekly Top-10
 // stack. `weeklyTopAvailable` flips to false when the API replies 404 (flag
 // off) so the tab disappears entirely.
 const [activeTab, setActiveTab] = usePersistentState<'discover' | 'top10'>('discover:activeTab', 'discover');
 const [weeklyTopAvailable, setWeeklyTopAvailable] = useState(true);

 // v1.2 session 14 — Task 1: post-match modal state. Opens when
 // `api.sendLike()` returns `{ isMutual: true, chat: { id }, isFirstMatch }`.
 // The founder-facing "confetti on first match" delight lives inside
 // MatchSuccessModal and fires when isFirstMatch=true.
 ...
 engagementTracker.commit(passedUser.id, 'left', { surface: 'discover' });
 // click-matrix.md §5 rank 9: was `.catch(() => {})` — failed passes were
 // invisible, so the algo lost the signal without the user knowing. Show a
 // toast on failure and roll back the pushAction so Undo still works.
 api.passUser(passedUser.id).catch((e) => {
  logError('discover.passUser', e);
  toast.error('Could not save pass', 'We may show them again.');
 });
 setBatchActed((n) => n + 1);
 ...
 engagementTracker.commit(currentUser.id, 'right', { surface: 'discover' });
 setBatchActed((n) => n + 1);
 try {
 ...
 engagementTracker.commit(currentUser.id, 'right', { surface: 'discover' });
 setBatchActed((n) => n + 1);
 // v1.2 session 14 — Task 1: read the mutual-match envelope shipped in
 // session 13. When `isMutual: true`, open MatchSuccessModal so the user
 // can pick a Move v2 opener; `isFirstMatch` drives the confetti burst.
 ...
 try {
   ...
   if (payload.isMutual) {
     setMatchModal({
       matchedUser: {
         id: target.id,
         displayName: target.displayName || 'your match',
         photo: target.photos?.[0]?.url || target.photos?.[0] || null,
       },
       chatId: payload.chat?.id ?? payload.match?.chatId ?? undefined,
       isFirstMatch: !!payload.isFirstMatch,
     });
     toast.love("It's a match!", `${target.displayName} liked you back`);
   } else {
     toast.love('Liked!', `${target.displayName} will see your like`);
   }
 } catch {
 ...
 engagementTracker.commit(currentUser.id, 'super', { surface: 'discover' });
 setBatchActed((n) => n + 1);
 ...
 engagementTracker.commit(tid, 'up', { surface: 'discover' });
 ...
 // v3.6.0 — Top-10 tab is its own self-contained surface; render it (with
 // the tab bar above) instead of any Discover-deck state.
 if (activeTab === 'top10' && weeklyTopAvailable) {
 return (
   <ErrorBoundary>
     <div className="h-full overflow-y-auto">
       <div className="max-w-[1080px] mx-auto px-6 pt-4 pb-6">
         <DiscoverTabs
           active={activeTab}
           onChange={(t) => { setActiveTab(t); track('discover.tab.changed', { to: t }); }}
           showTop10={weeklyTopAvailable}
         />
         <WeeklyTop10 onHide={() => { setWeeklyTopAvailable(false); setActiveTab('discover'); }} />
       </div>
     </div>
   </ErrorBoundary>
 );
...
/* ─── All Caught Up Terminal (v6.6) ─── */
 ...
 />
 <DiscoverFilterModal isOpen={showFilters} onClose={() => setShowFilters(false)} filters={filters} onApply={handleApplyFilters} />
 <DeferredPileModal
 surface="discover"
 isOpen={showDeferred}
 ...
 {/* v3.6.0 — Discover / Top-10 tab switcher */}
 <DiscoverTabs
   active={activeTab}
   onChange={(t) => { setActiveTab(t); track('discover.tab.changed', { to: t }); }}
   showTop10={weeklyTopAvailable}
 ...
 {/* Refresh — pulls a fresh ranked top-10 batch (mirrors DTM Refresh) */}
 <motion.button
 whileHover={!refreshing ? { scale: 1.02 } : undefined}
 whileTap={!refreshing ? { scale: 0.98 } : undefined}
 onClick={() => { void loadProfiles(); track('discover.refresh.clicked', {}); }}
 disabled={refreshing}
 ...
 : 'bg-white border-amber-200 text-amber-700 hover:border-amber-300 shadow-[0_2px_8px_rgba(180,140,40,0.08)]',
 )}
 >
 ...
 // v3.6.0 — "Show me less like this" from WhyCard. Fire pass-feedback
 // with the dedicated reason so the negative-signal pipeline learns,
 // then advance the deck via the standard pass flow.
 onLessLikeThis={() => {
   const u = currentUser;
   if (!u) return;
   api.passUserFeedback(u.id, 'less_like_this').catch(() => {});
   trackActivity('pass_feedback', 'profile', u.id, { reason: 'less_like_this' });
   handlePass();
 }}
 isActive={true}
 />
 ...
 {/* v1.2 session 14 — Task 1: MatchSuccessModal wired to the mutual-like
     envelope returned by POST /discover/like. Opens on isMutual=true;
     isFirstMatch drives the confetti (single-fire per user, guarded by
     Settings.hasSeenFirstMatch flipped inside the server transaction). */}
 {matchModal && (
 <MatchSuccessModal
   isOpen={true}
   onClose={() => setMatchModal(null)}
   matchedUser={matchModal.matchedUser}
   chatId={matchModal.chatId}
   isFirstMatch={matchModal.isFirstMatch}
   onSent={() => setMatchModal(null)}
 />
 )}
 </div>
 </ErrorBoundary>
...
/* ─── v3.6.0 Discover / Top-10 tab switcher ─── */
function DiscoverTabs({ active, onChange, showTop10 }: {
 active: 'discover' | 'top10';
 onChange: (t: 'discover' | 'top10') => void;
 showTop10: boolean;
}) {
 if (!showTop10) return null;
 const tabs: Array<{ id: 'discover' | 'top10'; label: string }> = [
   { id: 'discover', label: 'Discover' },
   { id: 'top10', label: '★ Top 10' },
 ];
 return (
   <div className="flex items-center gap-1 pt-3 pb-3" role="tablist" aria-label="Discover surfaces">
     {tabs.map((t) => (
       <button
         key={t.id}
         type="button"
         ...
         className={cn(
           'h-9 px-4 rounded-xl text-[13px] font-semibold transition-all',
           active === t.id
             ? 'bg-rose-main/10 text-rose-main shadow-sm'
             : 'text-text-muted hover:text-text-secondary hover:bg-miamo-surface/60',
         )}
       >
         {t.label}
       </button>
