...
export function ProfileCard({
 user, aiData, onPass, onMove, onLike, onSuperLike, onSeeLater, onLessLikeThis, isActive,
}: {
 ...
 onSuperLike?: () => void;
 onSeeLater?: () => void;
 // v3.6.0 — invoked when the user picks "Show me less like this" from the
 // WhyCard popover. Falls back to a server-side passUserFeedback fire if
 // omitted; the Discover page wires its own implementation that also
 // advances the deck.
 onLessLikeThis?: () => void;
 isActive: boolean;
}) {
 ...
 engagementTracker.onCardVisible(user.id, 'discover');
 ...
 // v8: within-card like (photo/prompt) counts as a strong depth signal.
 engagementTracker.onLike(user.id);
 };
 ...
 const dwellMs = performance.now() - hoverEnteredAt.current;
 cardTracker.onHover(user.id, dwellMs);
 if (dwellMs >= 250) engagementTracker.onHover(user.id);
 hoverEnteredAt.current = 0;
 }
 }}
 >
 {/* v3.6.0 — "Why am I seeing this?" overlay. Lives on the relative
     wrapper (not the overflow-hidden card body) so the popover can
     extend past the photo without being clipped. */}
 <WhyCard targetId={user.id} onLessLikeThis={onLessLikeThis} />
...
function ActionButton({
  ...
  return (
    <motion.button
      ...
      disabled={disabled}
      data-testid={testid}
      // click-matrix.md §5 rank 24 (a11y sweep of primary CTAs): screen readers
      // should read the semantic action, not just the two-word visible label.
