...
// ─── G.18 first-match delight ─────────────────────────────────────
// Confetti animation renders on the user's very first match only.
// State is tracked via Settings.hasSeenFirstMatch (server) + a local
// "already shown this session" flag. framer-motion drives the burst —
// no confetti library added (per session brief: no new npm deps).
const CONFETTI_COLORS = ['#C97856', '#D4896A', '#F4C275', '#8E5AB2', '#5CB5C7'];
function ConfettiBurst({ count = 24 }: { count?: number }) {
  const pieces = useMemo(() => Array.from({ length: count }).map((_, i) => ({
    ...
    y: -Math.random() * 60 - 30,       // -90%..-30% (upward)
    rot: Math.random() * 360,
    ...
    <div aria-hidden="true" className="pointer-events-none absolute inset-0 overflow-hidden">
      {pieces.map(p => (
        <motion.span
          ...
          animate={{ x: `${p.x}%`, y: `${p.y}%`, rotate: p.rot, opacity: [0, 1, 1, 0] }}
...
export function MatchSuccessModal({
  ...
  receiverHash,
  onSent,
  // G.18 — when true, the modal renders a confetti burst on mount. The
  // caller reads Settings.hasSeenFirstMatch from the server and passes
  // `isFirstMatch = !settings.hasSeenFirstMatch`, then PUTs
  // hasSeenFirstMatch=true immediately after opening so the delight
  // fires exactly once per user.
  isFirstMatch = false,
}: {
  isOpen: boolean;
  onClose: () => void;
      ...
      {isOpen && (
        <Portal>
          ...
          <motion.div
            ...
            role="dialog"
            aria-modal="true"
            aria-label={`It's a match with ${matchedUser.displayName}${isFirstMatch ? ' — your first match on Miamo' : ''}`}
          >
            {/* G.18 first-match delight — renders only on the very first match. */}
            {isFirstMatch && <ConfettiBurst />}
