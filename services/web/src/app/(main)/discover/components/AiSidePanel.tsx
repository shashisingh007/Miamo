  ...
  return (
    <div className="rounded-2xl card-premium overflow-hidden">
      <button onClick={() => setExpanded(!expanded)}
        className="w-full flex items-center justify-between px-5 py-4 text-left hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors">
        <span className="text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-[0.12em] flex items-center gap-2">
          <Sparkles className="w-3.5 h-3.5 text-lavender-400/60" /> {moves.length} more suggestions
        </span>
        <ChevronDown className={cn('w-4 h-4 text-gray-300 dark:text-gray-600 transition-transform', expanded && 'rotate-180')} />
      </button>
      <AnimatePresence>
        {expanded && (
                ...
                return (
                  <button key={i} onClick={() => !isSent && onSend(rec, idx)} disabled={isSent}
                    className={cn(
                      ...
                      isSent ? 'bg-emerald-400/[0.06] border-emerald-400/20' : 'bg-gray-50/50 dark:bg-gray-800/50 border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 hover:border-gray-200 dark:hover:border-gray-700',
                    )}>
                    <p className={cn('text-[12px] leading-relaxed', isSent ? 'text-emerald-300' : 'text-gray-600 dark:text-gray-400')}>{rec.text}</p>
                    <div className="flex items-center gap-3 mt-1.5">
                      <span className="text-[9px] text-gray-400 dark:text-gray-500 capitalize font-medium">{rec.type.replace(/-/g, ' ')}</span>
                      <div className="flex gap-[3px]">
                        {Array.from({ length: 5 }).map((_, j) => (
                          <div key={j} className={cn('w-[5px] h-[5px] rounded-full', j < Math.round(rec.confidence * 5) ? 'bg-white/40' : 'bg-gray-50 dark:bg-gray-800')} />
                        ))}
                      </div>
      ...
      <div className="space-y-3">
        {[1, 2, 3, 4].map(i => (
          <div key={i} className="rounded-2xl card-premium p-5">
            <div className="h-3 bg-gray-50 dark:bg-gray-800 rounded-full animate-pulse w-1/2 mb-4" />
            <div className="space-y-2.5">
              <div className="h-2.5 bg-gray-50 dark:bg-gray-800 rounded-full animate-pulse" />
              <div className="h-2.5 bg-gray-50 dark:bg-gray-800 rounded-full animate-pulse w-4/5" />
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="space-y-3">
      ...
      <motion.div
        ...
        className={`rounded-2xl bg-gradient-to-b ${scoreBg} to-transparent border border-gray-200 dark:border-gray-700 overflow-hidden`}
      >
        <div className="p-5">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-[13px] font-bold text-gray-900 dark:text-white tracking-wide">AI Compatibility</span>
            </div>
            <motion.span
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
                ...
                return (
                  <div key={key}>
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[11px] text-gray-400 dark:text-gray-500 font-semibold">{breakdownLabels[key] || key}</span>
                      <span className="text-[11px] text-gray-600 dark:text-gray-400 font-bold tabular-nums">{pct}%</span>
                    </div>
                    <div className="h-[5px] bg-gray-50 dark:bg-gray-800 rounded-full overflow-hidden">
                      <motion.div
                        initial={{ width: 0 }}
                        animate={{ width: `${pct}%` }}
      ...
      {aiData.whyThisMatch?.length > 0 && (
        ...
        >
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <span className="text-[13px] font-bold text-gray-900 dark:text-white tracking-wide">Why this match?</span>
          </div>
          <div className="space-y-3">
            {aiData.whyThisMatch.map((reason, i) => (
              ...
              >
                <div className="w-5 h-5 rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center flex-shrink-0 mt-0.5">
                  <span className="text-[9px] font-black text-gray-600 dark:text-gray-400">{i + 1}</span>
                </div>
                <p className="text-[12px] text-gray-600 dark:text-gray-400 leading-[1.7] font-medium">{reason}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
          ...
          <motion.button
            ...
            className={cn(
              ...
              isSent
                ...
                : 'bg-gray-50/50 dark:bg-gray-800/50 border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 hover:border-gray-200 dark:hover:border-gray-700 hover:shadow-md',
            )}
          >
            <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-[0.12em] mb-2">
              {isSent ? '✓ Move sent!' : labels[i] || `Suggested move`}
            </p>
            <p className={cn(
              'text-[14px] leading-[1.65] font-medium',
              isSent ? 'text-emerald-300' : 'text-gray-800 dark:text-gray-200',
            )}>
              {rec.text}
            </p>
            {!isSent && (
              <div className="flex items-center gap-2 mt-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                ...
                <span className="text-[10px] text-gray-400 dark:text-gray-500 font-semibold">Click to send</span>
              </div>
            )}
          </motion.button>
        );
      ...
      >
        <div className="flex items-center gap-2.5 mb-4">
          <div className="w-8 h-8 rounded-xl bg-gray-50 dark:bg-gray-800 flex items-center justify-center">
            <Shield className="w-4 h-4 text-gray-800 dark:text-gray-200" />
          </div>
          <span className="text-[13px] font-bold text-gray-900 dark:text-white tracking-wide">Trust & Safety</span>
        </div>
        <div className="space-y-2.5">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-[12px] text-gray-500 dark:text-gray-400 font-medium">
              {user.verified ? (
                <>Identity <span className="px-2 py-0.5 bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200 rounded-md text-[10px] font-bold ml-1">VERIFIED</span></>
              ) : 'Not yet verified'}
            </span>
          </div>
          <div className="flex items-center gap-2.5">
            <div className="w-2 h-2 rounded-full bg-emerald-400 shadow-[0_0_6px_rgba(52,211,153,0.4)]" />
            <span className="text-[12px] text-gray-500 dark:text-gray-400 font-medium">No reports</span>
          </div>
          {profile.profileScore && (
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-[12px] text-gray-500 dark:text-gray-400 font-medium">Profile {profile.profileScore}% complete</span>
            </div>
          )}
          {profile.online && (
            <div className="flex items-center gap-2.5">
              <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse shadow-[0_0_6px_rgba(52,211,153,0.6)]" />
              <span className="text-[12px] text-gray-500 dark:text-gray-400 font-medium">Currently online</span>
            </div>
          )}
        </div>
      </motion.div>
      ...
      >
        <div className="flex items-center gap-2.5 mb-3">
          <div className="w-8 h-8 rounded-xl bg-gray-50 dark:bg-gray-800 flex items-center justify-center">
            <MessageSquare className="w-4 h-4 text-gray-800 dark:text-gray-200" />
          </div>
          <span className="text-[13px] font-bold text-gray-900 dark:text-white tracking-wide">Write Your Move</span>
        </div>
        <div className="relative">
          <textarea
            ...
            className="w-full h-[60px] rounded-xl bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white text-[12px] px-4 py-3 pr-12 resize-none focus:border-pink-200 focus:outline-none placeholder:text-gray-400 dark:placeholder:text-gray-500 transition-colors"
          />
          <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}
            ...
            {customSent ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Send className="w-3.5 h-3.5 text-gray-800 dark:text-gray-200" />}
          </motion.button>
        </div>
        <p className="text-[10px] text-gray-300 dark:text-gray-600 mt-2 font-medium">Or send blank — just press send</p>
      </motion.div>
    </div>
  );
}
