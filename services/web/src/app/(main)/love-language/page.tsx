...
const LANGUAGES = [
 ...
 color: 'from-rose to-rose-dark', bg: 'bg-miamo-surface', accent: 'text-rose',
...
function ResultCard({ primary, secondary, scores }: { primary: typeof LANGUAGES[0]; secondary: typeof LANGUAGES[0]; scores: Record<string, number> }) {
 ...
 {/* Hero */}
 <div className={cn('p-8 text-center text-text-primary bg-gradient-to-br', primary.color)}>
 ...
 <p className="text-text-primary/80 text-sm">{primary.desc}</p>
 </div>

 <div className="p-6 space-y-5">
 {/* Score bars */}
 <div className="space-y-3">
 <h3 className="font-bold text-text-primary text-sm">Your Love Language Profile</h3>
 {LANGUAGES.map(l => {
 ...
 <span className={cn('text-xs font-semibold', isPrimary ? l.accent : 'text-text-muted')}>{l.name}</span>
 ...
 className={cn('h-full rounded-full', isPrimary ? `bg-gradient-to-r ${l.color}` : 'bg-border-light')} />
 </div>
 </div>
 </div>
 );
 ...
 {/* Secondary */}
 <div className="flex items-center gap-3 p-3 rounded-xl bg-miamo-surface">
 <SecIcon className={cn('w-5 h-5', secondary.accent)} />
 <div>
 <p className="text-xs text-text-muted">Secondary Language</p>
 <p className="text-sm font-bold text-text-secondary">{secondary.name} {secondary.emoji}</p>
 </div>
 </div>
 </div>
 </Card>
...
export default function LoveLanguagePage() {
 ...
 <div>
 <h1 className="text-2xl font-black text-text-primary">Love Language</h1>
 <p className="text-sm text-text-muted">Discover how you give and receive love 💝</p>
 </div>
 </div>
 </motion.div>
 ...
 <h2 className="text-2xl font-black text-text-primary mb-2">What&apos;s Your Love Language?</h2>
 ...
 <p className="text-[9px] font-bold text-text-muted mt-1">{l.name.split(' ')[0]}</p>
 </motion.div>
 ))}
 </div>
 ...
 <span className="text-xs font-bold text-rose">Question {qIdx + 1} of {QUESTIONS.length}</span>
 <span className="text-xs text-text-muted">{Math.round((qIdx / QUESTIONS.length) * 100)}%</span>
 </div>
 <div className="h-2 bg-miamo-surface rounded-full overflow-hidden">
 ...
 className="text-xl font-black text-text-primary mb-6 text-center">{QUESTIONS[qIdx].q}</motion.h2>
 ...
 className="w-full text-left p-4 rounded-xl border-2 border-border hover:border-rose-light hover:bg-miamo-surface/50 transition-all font-semibold text-text-secondary">
 {a.text}
 </motion.button>
 ))}
 </div>
