...
import { filterNavByDisclosure, isProgressiveDisclosureEnabled, type DisclosureUser } from '@/lib/progressiveDisclosure';
...
export default function MainLayout({ children }: { children: React.ReactNode }) {
 ...
 // The server-side logout revokes the refresh cookie. If it fails (network,
 // 5xx, timeout), we still tear down the client-side session below — but the
 // user needs to see WHY the network call didn't complete cleanly and Sentry
 // needs the error to catch a broken auth deploy.
 try {
  await api.logout();
 } catch (e) {
  logError('layout.logout', e);
  toast.error("Couldn't reach the server to sign out — clearing local session anyway.");
 }
 clearAuth();
 window.location.href = '/login';
 };
 ...
 // ─── G.18 progressive-disclosure wire (Task 1a) ─────────────────────
 // When FEATURE_PROGRESSIVE_DISCLOSURE_ENABLED=1 (or NEXT_PUBLIC_ variant)
 // filter DTM / Family-Brief / Weekly-Top-10 / AI-Match / Vibe-Check /
 // Creativity nav items behind the disclosure rules. Default OFF → both
 // NAV_MAIN and NAV_SECONDARY render exactly as v1 (bit-identical).
 const disclosureEnabled = isProgressiveDisclosureEnabled();
 const disclosureUser: DisclosureUser | null = profile
   ? {
       matchCount: profile?.stats?.matchCount ?? profile?.matchCount ?? null,
       signupAt: profile?.user?.createdAt ?? profile?.createdAt ?? null,
       intent: (profile?.user?.profile?.datingIntent ?? profile?.profile?.datingIntent ?? null) as DisclosureUser['intent'],
       onboardingComplete: (profileScore ?? 0) >= 75,
       seriousModeEnabled: !!(profile?.settings?.seriousModeEnabled ?? profile?.user?.settings?.seriousModeEnabled ?? profile?.user?.profile?.seriousMode ?? false),
     }
   : null;
 const navMain = filterNavByDisclosure(NAV_MAIN, disclosureUser, { enabled: disclosureEnabled });
 const navSecondary = filterNavByDisclosure(NAV_SECONDARY, disclosureUser, { enabled: disclosureEnabled });

 return (
 <div className="flex h-screen overflow-hidden bg-miamo-bg relative">
 {/* click-matrix.md §5 rank 46-60 (a11y / WCAG AA): skip-link. Hidden by
     default; becomes visible on keyboard focus so keyboard-only users can
     jump past the sidebar/header straight to the main region. */}
 <a
   href="#main-content"
   className="sr-only focus:not-sr-only focus:fixed focus:top-2 focus:left-2 focus:z-[100] focus:px-3 focus:py-2 focus:rounded-md focus:bg-rose-main focus:text-white focus:shadow-lg focus:outline-none focus:ring-2 focus:ring-white/60"
 >
 ...
 <aside className="hidden lg:flex flex-col w-[270px] glass-sidebar relative z-10">
          ...
          <div className="flex items-center justify-between h-[80px] px-6">
            ...
            <Link href="/premium" className="group flex flex-col items-center gap-1">
 ...
 {/* ═══ MAIN CONTENT ═══ */}
 <main id="main-content" tabIndex={-1} className="flex-1 flex flex-col min-h-0 relative z-10" style={{ overflow: 'clip' }}>
 {/* Premium Header */}
 <header
 ...
 {pathname.startsWith('/messages') || pathname.startsWith('/beats') || pathname.startsWith('/videos') ? (
 children
