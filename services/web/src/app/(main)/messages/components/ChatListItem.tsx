...
export function ChatListItem({ chat, active, onClick, onAction, selectMode, selected, onSelect }: { chat: any; active: boolean; onClick: () => void; onAction: (action: string, data?: any) => void; selectMode?: boolean; selected?: boolean; onSelect?: () => void }) {
 ...
 active ? 'bg-rose-main/10 border border-rose-main/20' : 'hover:bg-miamo-elevated/50',
 selected && 'bg-rose-main/5 border border-rose-main/30',
 (chat.held || chat.onHold || chat._isHeld) && !active && 'opacity-40'
 )}>
 {selectMode && (
 <div className="shrink-0">
 {selected ? <CheckSquare className="w-5 h-5 text-rose-main" /> : <Square className="w-5 h-5 text-text-muted/40" />}
 </div>
 )}
 ...
 {chat.unreadCount > 0 && (
 <span className="shrink-0 w-5 h-5 bg-rose-main text-text-primary text-[10px] font-bold rounded-full flex items-center justify-center">{chat.unreadCount}</span>
 )}
 </div>
 </div>
