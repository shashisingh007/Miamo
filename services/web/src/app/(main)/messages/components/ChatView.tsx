...
function CallOverlay({ type, user, onEnd }: { type: 'voice' | 'video'; user: any; onEnd: () => void }) {
 ...
 {status === 'ringing' && <div className="absolute inset-0 rounded-full border-2 border-rose-main/30 animate-ping" />}
 </div>
...
export function ChatView({ chat, onBack, onRefreshChats, onReport, onUnmatch, onBlock }: { chat: any; onBack: () => void; onRefreshChats: () => void; onReport: () => void; onUnmatch: () => void; onBlock: () => void }) {
 ...
 className="shrink-0 w-10 h-10 rounded-xl flex items-center justify-center text-text-primary font-bold transition-all duration-200 hover:-translate-y-0.5 active:scale-95"
