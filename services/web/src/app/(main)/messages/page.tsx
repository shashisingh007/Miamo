...
import {
 ...
 Shield, Lock, EyeOff, ListChecks, PlayCircle, Unlink, Mic, X as XIcon,
} from 'lucide-react';
...
function GlassTooltipButton({ label, active, activeColor = 'lavender', onClick, children }: { label: string; active?: boolean; activeColor?: 'lavender' | 'amber'; onClick: () => void; children: React.ReactNode }) {
 ...
 >
 <span className="px-2 py-1 text-[10px] font-medium text-text-secondary whitespace-nowrap rounded-lg bg-miamo-card/5 backdrop-blur-md border border-white/10 shadow-lg">
 {label}
 </span>
 </motion.div>
 )}
...
> {
 ...
 <MessageCircle className="w-10 h-10 text-rose-main/40 mx-auto" />
 ...
 className="px-4 py-2 bg-rose-main text-text-primary rounded-lg font-medium text-sm">
 Refresh Page
 </button>
 </div>
 </div>
...
function MessagesPageInner() {
 ...
 // v3.6.0 — Voice fingerprint reveal. Shown one-time after the user has sent
 // 50+ text messages and gated server-side behind FEATURE_VOICE_FINGERPRINT_ENABLED.
 const [voiceFpToastShown, setVoiceFpToastShown] = useState(false);
 const [voiceFpModalOpen, setVoiceFpModalOpen] = useState(false);
 const toast = useToast();

 // Wraps a bulk chat action (unmatch/clear/archive/mute/pin/resume) so any
 // per-row failure surfaces as a single toast the user can react to instead
 // of silently dropping (audit finding: `messages/page.tsx:335-362` had 8
 // empty catches). We collect per-id errors and report the count; the first
 // real Error is forwarded to Sentry via `logError` so a broken deploy
 // shows up in dashboards even when only some rows fail.
 const runBulkChatAction = useCallback(
   async (label: string, ids: string[], fn: (id: string) => Promise<unknown>) => {
     ...
     if (failed > 0) {
       logError(`messages.bulk.${label}`, firstError);
       toast.error(`Couldn't ${label} ${failed} of ${ids.length} chat${ids.length === 1 ? '' : 's'} — try again.`);
     }
   },
   [toast]
 );
 ...
 // v3.6.0 — Voice fingerprint discovery toast.
 // Probe the endpoint once; if the user has 50+ outbound messages AND has not
 // already dismissed the prompt, surface a one-time toast on the messages page.
 // Endpoint is 404 when FEATURE_VOICE_FINGERPRINT_ENABLED is off — we treat
 // that as "feature disabled" and stay silent.
 useEffect(() => {
 if (typeof window === 'undefined') return;
 if (voiceFpToastShown) return;
 try {
   if (localStorage.getItem('miamo:voice_fp_dismissed') === '1') return;
 } catch { /* ignore */ }
 let cancelled = false;
 api.getMyVoiceFingerprint()
   .then((r) => {
     if (cancelled) return;
     if ((r?.data?.sentMessageCount ?? 0) >= 50) setVoiceFpToastShown(true);
   })
   .catch(() => { /* 404 (flag off) or auth issue — stay silent */ });
 return () => { cancelled = true; };
 }, [voiceFpToastShown]);

 const dismissVoiceFpToast = () => {
 setVoiceFpToastShown(false);
 try { localStorage.setItem('miamo:voice_fp_dismissed', '1'); } catch { /* ignore */ }
 };
 ...
 {loading ? (
 <div className="flex justify-center py-12"><img src="/assets/logo.svg" alt="" className="w-8 h-8 rounded-lg animate-pulse" /></div>
 ...
 <button onClick={() => { setSelectedChats(new Set(filteredChats.map(c => c.id))); }} className="text-[11px] text-rose-main hover:underline">Select all</button>
 </div>
 <div className="flex items-center gap-1.5 flex-wrap">
 {tab === 'held' ? (
 <>
 <button onClick={async () => {
 // Resume all selected: for each held chat, look up the other user and
 // call the resume endpoint. `runBulkChatAction` surfaces a toast if
 // any of the calls fails so users don't stare at an unchanged list.
 const ids = Array.from(selectedChats);
 const userIds = ids
   .map(id => {
     const chat = chats.find(ch => ch.id === id);
     return (chat?.otherUser || chat?.user1)?.id as string | undefined;
   })
   .filter((u): u is string => Boolean(u));
 await runBulkChatAction('resume', userIds, (uid) => api.resumeIncoming(uid));
 await syncHeldFromBackend();
 updateHeldChats(prev => { const n = new Set(prev); ids.forEach(id => n.delete(id)); return n; });
 ...
 <button onClick={async () => { await runBulkChatAction('unmatch', Array.from(selectedChats), (id) => api.unmatch(id, 'bulk_unmatch')); setSelectedChats(new Set()); setSelectMode(false); loadChats(); }}
 ...
 <button onClick={async () => { await runBulkChatAction('delete', Array.from(selectedChats), (id) => api.clearChat(id)); setSelectedChats(new Set()); setSelectMode(false); loadChats(); }}
 className="flex-1 flex items-center justify-center gap-1.5 h-8 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 text-[11px] font-medium hover:bg-red-500/20 transition-colors">
 ...
 <button onClick={async () => { await runBulkChatAction('archive', Array.from(selectedChats), (id) => api.archiveChat(id)); setSelectedChats(new Set()); setSelectMode(false); loadChats(); }}
 ...
 <button onClick={async () => { await runBulkChatAction('mute', Array.from(selectedChats), (id) => api.muteChat(id, true)); setSelectedChats(new Set()); setSelectMode(false); loadChats(); }}
 className="flex-1 flex items-center justify-center gap-1.5 h-8 rounded-lg bg-miamo-elevated border border-border/30 text-text-secondary text-[11px] font-medium hover:bg-miamo-card transition-colors">
 <VolumeX className="w-3 h-3" /> Mute
 </button>
 <button onClick={async () => { await runBulkChatAction('pin', Array.from(selectedChats), (id) => api.pinChat(id, true)); setSelectedChats(new Set()); setSelectMode(false); loadChats(); }}
 className="flex-1 flex items-center justify-center gap-1.5 h-8 rounded-lg bg-miamo-elevated border border-border/30 text-text-secondary text-[11px] font-medium hover:bg-miamo-card transition-colors">
 ...
 {/* ─── v3.6.0 — Voice Fingerprint reveal toast + modal ─── */}
 <AnimatePresence>
 {voiceFpToastShown && !voiceFpModalOpen && (
 <motion.div
   key="voice-fp-toast"
   initial={{ opacity: 0, y: 24 }}
   animate={{ opacity: 1, y: 0 }}
   exit={{ opacity: 0, y: 24 }}
   className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 max-w-sm w-[90%]"
 >
   <div className="bg-miamo-card border border-rose-main/30 rounded-2xl shadow-xl p-3 flex items-center gap-3">
     <div className="w-10 h-10 rounded-full bg-rose-main/15 flex items-center justify-center shrink-0">
       <Mic className="w-5 h-5 text-rose-main" />
     </div>
     <div className="flex-1 min-w-0">
       <p className="text-[12px] font-bold text-text-primary">See your voice fingerprint?</p>
       <p className="text-[11px] text-text-muted truncate">A statistical reveal of how you write.</p>
     </div>
     <button
       onClick={() => { setVoiceFpModalOpen(true); setVoiceFpToastShown(false); }}
       className="shrink-0 h-9 px-3 rounded-lg bg-rose-main text-text-primary text-[11px] font-bold hover:bg-rose-main/90 transition-colors"
     >
       Reveal
     </button>
     <button
       onClick={dismissVoiceFpToast}
       aria-label="Dismiss"
       className="shrink-0 w-8 h-8 rounded-full bg-miamo-surface flex items-center justify-center text-text-muted hover:text-text-primary"
     >
 ...
 <VoiceFingerprint
 isOpen={voiceFpModalOpen}
 onClose={() => { setVoiceFpModalOpen(false); try { localStorage.setItem('miamo:voice_fp_dismissed', '1'); } catch {} }}
 />
