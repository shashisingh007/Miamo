...
export default function CompatibilityPage() {
 ...
 useEffect(() => { api.getMatches().then(r => setMatches(r.data || [])).catch((e) => logError('compatibility.getMatches', e)); }, []);

 // click-matrix.md §5 rank 3: the Share Results button was wired to resetQuiz (a lie).
 // Use the Web Share API when available, else copy a summary to the clipboard.
 const shareResults = async () => {
  if (sharing) return;
  const matchName = (selectedMatch?.matchedUser || selectedMatch)?.displayName || 'my match';
  const shareText = `Miamo says ${matchName} and I are ${score}% compatible! ${
   score >= 80 ? '🔥 Soulmate level.' : score >= 60 ? '✨ Great chemistry.' : score >= 40 ? '💫 Potential is there.' : '🌱 Growing match.'
  }`;
  setSharing(true);
  try {
   try { track('compat.share', { score }); } catch {}
   if (typeof navigator !== 'undefined' && typeof navigator.share === 'function') {
    await navigator.share({ title: 'Miamo compatibility', text: shareText });
    toast.success('Shared!');
   } else if (typeof navigator !== 'undefined' && navigator.clipboard) {
    await navigator.clipboard.writeText(shareText);
    toast.success('Copied to clipboard', 'Paste it wherever you like.');
   } else {
    toast.info('Sharing not supported on this device.');
   }
  } catch (e: any) {
   // AbortError = user dismissed share sheet; not an error worth surfacing.
   if (e?.name !== 'AbortError') {
    logError('compatibility.share', e);
 ...
 <Button variant="secondary" onClick={resetQuiz} className="flex-1 gap-2" aria-label="Try again"><RefreshCw className="w-4 h-4" /> Try Again</Button>
 <Button onClick={shareResults} loading={sharing} className="flex-1 gap-2" aria-label="Share compatibility results"><Heart className="w-4 h-4" /> Share Results</Button>
 </div>
 </div>
 </Card>
 </motion.div>
