...
const MOODS = [
  ...
  { id: 'flirty', emoji: '😏', label: 'Flirty', color: 'from-pink-400 to-rose-500', wave: '#f472b6' },
  ...
  { id: 'meh', emoji: '😐', label: 'Meh', color: 'from-gray-400 to-slate-500', wave: '#94a3b8' },
  { id: 'romantic', emoji: '🥰', label: 'Romantic', color: 'from-rose-400 to-pink-500', wave: '#fb7185' },
];
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      {[...Array(count)].map((_, i) => (
        <motion.div
          key={i}
          className="absolute w-2 h-2 rounded-full bg-gradient-to-r from-pink-300 to-violet-300"
        ...
        {done && selectedMood && (
          <motion.div key="result" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ type: 'spring' }}>
            <Card className="p-0 overflow-hidden relative">
              <div className={cn('p-10 text-center text-white bg-gradient-to-br relative', selectedMood.color)}>
                <Particles count={20} />
                ...
                <div className="relative z-10">
                  ...
                  <p className="text-white/70 text-lg">Looking for {intent}</p>
                  <div className="flex items-center justify-center gap-1 mt-3">
                    {[...Array(5)].map((_, i) => (
                      <motion.div key={i} initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.5 + i * 0.1 }}
                        className={cn('w-6 h-6 rounded-full', i < energy ? 'bg-white/80' : 'bg-white/20')} />
                    ))}
                    <span className="ml-2 text-sm font-bold text-white/80">Energy</span>
                  </div>
                </div>
              </div>
