...
export function ReelsView({ activeCat, categories, onOpenComments, onOpenMove, showToast, refreshBalance, source = 'reels', sort = 'trending', search = '' }: Props) {
  ...
  // click-matrix.md §3.2 + §5 rank 6: replaced window.prompt for report reason
  // with a proper InputModal.
  ...
  const onLike = async () => {
    ...
    try {
      ...
      // click-matrix.md §5 ranks 16-25: roll back optimistic like on failure.
      logError('creativity.like', e);
      cache.setData((prev) => prev ? {
        ...prev,
        items: prev.items.map((p, i) => i === idx ? { ...p, viewer: { ...p.viewer, liked: wasLiked }, counts: { ...p.counts, beats: p.counts.beats + (wasLiked ? 1 : -1) } } : p),
      } : prev);
      showToast('Could not save like — please try again');
    }
  };

  const onBeat = onLike; // beat == positive reaction in current model

  // click-matrix.md §5 ranks 16-25: don't hide the card if the dislike POST
  // failed — the user's negative signal is dropped and they'll see the same
  // card again on the next fetch with no clue why.
  const onDislike = async () => {
    if (!current) return;
    try {
      await api.dislikeCreativityItem(current.id);
  ...
  const onSave = async () => {
    if (!current) return;
    const wasSaved = current.viewer.saved;
    cache.setData((prev) => prev ? {
      ...prev,
    ...
    try {
      await api.saveCreativityItem(current.id);
      showToast(wasSaved ? 'Removed' : 'Saved');
    } catch (e) {
      // click-matrix.md §5 ranks 16-25: roll back optimistic save on failure.
      logError('creativity.save', e);
      cache.setData((prev) => prev ? {
        ...prev,
        items: prev.items.map((p, i) => i === idx ? { ...p, viewer: { ...p.viewer, saved: wasSaved }, counts: { ...p.counts, saves: p.counts.saves + (wasSaved ? 1 : -1) } } : p),
      } : prev);
      showToast('Could not save — please try again');
    }
  };

  // click-matrix.md §5 rank 18 (ReelsView share): the old code fired the toast
  // BEFORE awaiting the clipboard write; on slow clipboards this misled the
  // user. Also both catches were silent. Await clipboard first; only then toast.
  const onShare = async () => {
    if (!current) return;
    try {
      await api.shareCreativityItem(current.id, 'app');
    } catch (e) {
      logError('creativity.share', e);
      // Non-fatal — the copy step still runs so the user gets a usable link.
    }
    let copied = false;
    try {
      ...
      logError('creativity.share.clipboard', e);
    }
    showToast(copied ? 'Link copied' : 'Could not copy — try again');
    refreshBalance();
  };
  ...
  // click-matrix.md §5 ranks 16-25: was silent-failing. If the negative-signal
  // POST fails, don't hide the card (ranker never learned) and tell the user.
  const onNotInterested = async () => {
    if (!current) return;
    try {
      await api.notInterestedCreativityItem(current.id, 'not_interested');
      showToast('Hidden — we\u2019ll learn from this');
      cache.setData((prev) => prev ? { ...prev, items: prev.items.filter((_, i) => i !== idx) } : prev);
      setIdx((i) => Math.min(i, Math.max(0, items.length - 2)));
    } catch (e) {
      logError('creativity.not_interested', e);
      showToast('Could not save that — please try again');
    }
    setMoreOpen(false);
  };

  // click-matrix.md §3.2 + §5 rank 6: open the report InputModal instead of
  // firing a native window.prompt.
  const onReport = () => {
    if (!current) return;
    setReportOpen(true);
    setMoreOpen(false);
  ...
  const submitReport = async (reason: string) => {
    if (!current) return;
    try {
      await api.reportCreativityItem(current.id, reason);
      showToast('Reported. Thank you.');
    } catch (e) {
      logError('creativity.report', e);
      showToast('Could not send report');
    }
  };

  // click-matrix.md §5 ranks 16-25: was silent-failing on hide-author.
  const onHideAuthor = async () => {
    if (!current) return;
    try {
      await api.hideCreativityAuthor(current.id);
      ...
      logError('creativity.hide_author', e);
      showToast('Could not hide — please try again');
    }
    setMoreOpen(false);
  };
  ...
  return (
    <div className="mx-auto flex max-w-3xl items-center justify-center gap-3 sm:gap-4">
      ...
      {/* click-matrix.md §5 rank 6: report reason input (replaces window.prompt). */}
      <InputModal
        open={reportOpen}
        onClose={() => setReportOpen(false)}
        onSubmit={submitReport}
        title="Report this post"
        label="Reason (spam, abuse, harassment, etc.)"
