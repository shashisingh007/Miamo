...
class ApiClient {
 ...
 // Auth
 // v3.7 — register accepts EITHER `identifier` (auto-detected email/phone),
 // `email`, or `phone`. Legacy call-sites passing `{email, password, ...}`
 // continue to work: the server side accepts all three shapes.
 async register(data: { identifier?: string; email?: string; phone?: string; password: string; displayName: string }) {
 ...
 // v3.7 — OTP-only signup completion (no password set). Requires the
 // verifiedToken from /signup/verify. The user can add a password later
 // via forgot-password.
 async signupOtpComplete(data: { verifiedToken: string; displayName: string }) {
 return this.request<any>('/api/v1/auth/signup/otp-complete', { method: 'POST', body: JSON.stringify(data) });
 }
 ...
 // v3.7 — login accepts EITHER `identifier` (auto-detected), `email`, or
 // `phone`. Legacy `{email, password}` callers keep working.
 async login(data: { identifier?: string; email?: string; phone?: string; password: string }) {
 ...
 // v3.7 — passwordless login for an existing account. Stage 1 (`start`)
 // returns an otpToken bound to the user + device fingerprint; stage 2
 // (`verify`) exchanges otpToken + 6-digit code for session tokens.
 async loginOtpStart(data: { identifier: string }) {
 return this.request<any>('/api/v1/auth/login/otp/start', { method: 'POST', body: JSON.stringify(data) });
 }
 async loginOtpVerify(data: { otpToken: string; code: string }) {
 return this.request<any>('/api/v1/auth/login/otp/verify', { method: 'POST', body: JSON.stringify(data) });
 }
 async sendEmailOtp() {
 ...
 // Phase F — response now carries `nextRefreshAt` (ISO) + `secondsUntilRefresh`
 // (server-authoritative, avoids clock skew). Both are always present so the
 // client can render the countdown even when `data` is empty.
 async getWeeklyTop(): Promise<{
   ...
   try {
     ...
     return {
       ...
       secondsUntilRefresh: typeof raw.secondsUntilRefresh === 'number' ? raw.secondsUntilRefresh : null,
     };
   } catch (err) {
 ...
 // Phase F — typed-confirm gate. Callers MUST send `confirm: "DELETE"`.
 // Optional `confirmUsername` cross-checks against the server. The prior
 // no-body variant is preserved via a defaulted argument so existing callers
 // don't break; the server enforces the confirm gate regardless.
 async deleteAccount(payload: { confirm: 'DELETE'; confirmUsername?: string; reason?: string } = { confirm: 'DELETE' }) {
  return this.request<any>('/api/v1/settings/delete', { method: 'DELETE', body: JSON.stringify(payload) });
 }
 ...
 // Phase F — trust-score breakdown (flag-gated). 404 while the flag is OFF.
 async getTrustScore() { return this.request<any>('/api/v1/profiles/me/trust'); }
 // Phase F — Family Brief share dashboard (flag-gated).
 async getFamilyBriefShares() { return this.request<any>('/api/v1/dtm/family-brief/shares'); }
 ...
 // Safety
 // Phase F — enriched reportUser. Accepts optional `reasonId` (canonical
 // 12-reason list) + `evidence` (URL/text) alongside the legacy free-form
 // `reason`. Server treats reasonId as authoritative when present.
 async reportUser(data: { reportedId?: string; reason: string; reasonId?: string; details?: string; targetType?: 'user' | 'message' | 'photo' | 'story' | 'post' | 'creativity'; targetId?: string; evidence?: string }) { return this.request<ApiResponse<unknown>>('/api/v1/safety/report', { method: 'POST', body: JSON.stringify(data) }); }
 ...
 async getStoryLikes(id: string) { return this.request<any>(`/api/v1/stories/${id}/likes`); } async openChatWith(userId: string) { return this.request<any>(`/api/v1/messages/chats/with/${userId}`, { method: 'POST' }); }
 async getUserById(id: string) { return this.request<any>(`/api/v1/users/${id}`); }

 // ─── v1.2 (session 13) additions ───
 async dtmMutualInterest(targetUserId: string) {
  return this.request<any>('/api/v1/dtm/mutual-interest', { method: 'POST', body: JSON.stringify({ targetUserId }) });
 }
 async getIntentStatus() { return this.request<any>('/api/v1/settings/intent-status'); }
 async setIntentOverride(override: string | null) {
  return this.request<any>('/api/v1/settings/intent-override', { method: 'PUT', body: JSON.stringify({ override }) });
 }
 async getAdminFairnessGini() { return this.request<any>('/api/v1/admin/fairness-gini'); }
}
