...
import {
 ...
 type V8Surface,
 type V8MoveTone,
 type V8LanguageFamily,
} from '@/lib/track/v8Emit';

export type { V8Surface, V8MoveTone, V8LanguageFamily } from '@/lib/track/v8Emit';
...
// ─── v8 (v3.6.0) typed hooks ────────────────────────────────────────
// Thin React adapters that wrap the pure `emit*` functions in
// `@/lib/track/v8Emit`. Each returns a stable callback (useCallback)
// so it can be passed into effect deps without retrigger. The pure
// emitters are tested directly under `tests/v8-web-emits.test.ts`.

/**
 * Hook wrapper around `emitMoveAccepted`. The server-side messaging
 * route also ratifies this on send so the KPI isn't trusted to a
 * client-only emit.
 */
export function useTrackMoveAccepted() {
 return useCallback((receiverHash: string, slotIndex: number, hookCategory: string, tone: V8MoveTone) => {
  emitMoveAccepted(receiverHash, slotIndex, hookCategory, tone);
 }, []);
}
