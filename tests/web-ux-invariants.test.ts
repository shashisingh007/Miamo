...
describe('web UX invariants (click-matrix.md §5 Wave 1)', () => {
  ...
  it('Settings dead buttons now have onClick handlers or mailto: hrefs (§5 rank 1)', () => {
    const path = join(WEB_ROOT, 'app', '(main)', 'settings', 'page.tsx');
    const content = readFileSync(path, 'utf8');
    // Cancel subscription now routes to /premium.
    expect(content).toMatch(/Cancel subscription[\s\S]*?router\.push\(['"]\/premium['"]\)/);
    // Email us + Share (feedback) now open mailto: links.
    expect(content).toMatch(/mailto:support@miamo\.app/);
    expect(content).toMatch(/mailto:feedback@miamo\.app/);
    // What's new + Rate Miamo route to /coming-soon (real page).
    // Check the sparkles-icon row and heart-icon row.
    expect(content.match(/router\.push\(['"]\/coming-soon['"]\)/g)?.length ?? 0).toBeGreaterThanOrEqual(2);
  });
